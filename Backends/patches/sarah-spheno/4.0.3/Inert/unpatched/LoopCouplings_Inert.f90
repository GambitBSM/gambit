...
Real(dp) Function AlphaEwDR(Q,MVWp,MHp,MFd,MFu,MFe) 
 
Real(dp),Intent(in)::Q,MVWp,MHp(2),MFd(3),MFu(3),MFe(3)
Integer::i1 
Real(dp)::DeltaAlpha 
If (MZ_input) then 
...
Do i1=2,2
If (MHp(i1).gt.Q) Then 
DeltaAlpha=DeltaAlpha-1._dp/3._dp*Log(MHp(i1)/ Q)*Alpha/(2._dp*pi)  
End if 
End Do 
Do i1=1,3
If (MFd(i1).gt.Q) Then 
...
Do i1=2,2
If (MHp(i1).gt.Q) Then 
DeltaAlpha=DeltaAlpha+1._dp/3._dp*Log(MHp(i1)/ Q) 
End if 
End Do 
Do i1=1,3
If (MFd(i1).gt.Q) Then 
...
Real(dp) Function AlphaEW_T(AlphaEW_In, Q,MVWp,MHp,MFd,MFu,MFe) 
 
Real(dp),Intent(in)::AlphaEW_In, Q,MVWp,MHp(2),MFd(3),MFu(3),MFe(3)
Integer::i1 
Real(dp)::DeltaAlpha 
...
Subroutine DeltaVB(sinW2,sinW2_dr,rho,MFe,MG0,Mhh,MHp,Ye,ZEL,ZER,ZP,res)

Implicit None 
Real(dp),Intent(in) :: MFe(3),MG0,Mhh,MHp(2),ZP(2,2)

Complex(dp),Intent(in) :: Ye(3,3),ZEL(3,3),ZER(3,3)

Real(dp) :: MFe2(3),MG02,Mhh2,MHp2(2)

Complex(dp) :: cplcFeFeG0L(3,3),cplcFeFeG0R(3,3),cplcFeFehhL(3,3),cplcFeFehhR(3,3),cplcFeFvcHpL(3,3,2),& 
& cplcFeFvcHpR(3,3,2),cplcFvFeHpL(3,3,2),cplcFvFeHpR(3,3,2)
...
Do i1=1,3
  Do i2=1,2
If ((MFe2(i1).gt.mf_l2(2)).Or.(MHp2(i2).gt.mf_l2(2))) Then
coup1L = cplcFeFvcHpL(i1,gt1,i2)
coup1R = cplcFeFvcHpR(i1,gt1,i2)
coup2R = Conjg(cplcFeFvcHpL(i1,gt2,i2))
coup2L = Conjg(cplcFeFvcHpR(i1,gt2,i2))
sumI = sumI + chargefactor*0.5_dp*coup1L*coup2R*B1(0._dp,MFe2(i1),MHp2(i2))  
End if 
   End Do
  End Do
...
chargefactor = 1 
  Do i2=1,3
If ((MG02.gt.mf_l2(2)).Or.(MFe2(i2).gt.mf_l2(2))) Then
coup1L = cplcFeFeG0L(i2,gt1)
coup1R = cplcFeFeG0R(i2,gt1)
coup2R = Conjg(cplcFeFeG0L(i2,gt2))
coup2L = Conjg(cplcFeFeG0R(i2,gt2))
sumI = sumI + chargefactor*0.5_dp*coup1L*coup2R*B1(0._dp,MFe2(i2),MG02)  
End if 
  End Do
...
Do i1=1,2
  Do i2=1,3
If ((MHp2(i1).gt.mf_l2(2)).Or.(0._dp.gt.mf_l2(2))) Then
coup1L = cplcFvFeHpL(i2,gt1,i1)
coup1R = cplcFvFeHpR(i2,gt1,i1)
coup2R = Conjg(cplcFvFeHpL(i2,gt2,i1))
coup2L = Conjg(cplcFvFeHpR(i2,gt2,i1))
sumI = sumI + chargefactor*0.5_dp*coup1L*coup2R*B1(0._dp,0._dp,MHp2(i1))  
End if 
   End Do
  End Do
...
Implicit None 
Complex(dp),Intent(in) :: ratFd(3),ratFe(3),ratFu(3),ratHp(2),ratVWp

Real(dp),Intent(in) :: MFd(3),MFe(3),MFu(3),MHp(2),MVWp
...
Do i1 = 2 , 2
coup = coup + 1*(1)**2*ratHp(i1)*A_zero(mH2p/MHp(i1)**2)
HPPloopHp(i1) = HPPloopHp(i1)+1*(1)**2*ratHp(i1)*A_zero(mH2p/MHp(i1)**2)
End Do 
Do i1 = 1 , 3
...
End Module LoopCouplings_Inert 
