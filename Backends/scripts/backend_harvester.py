...
#    backend_types_rollcall_<Module>.hpp  (one per module)
#
...
// All backend type definitions (must precede the frontend headers, which use these types).
...
"""
    ...
    # Generate per-module backend type rollcall files.
    bits_per_backend = compute_bits_per_backend(".", "./Backends/include/gambit/Backends/frontends")

    # Invert to bit → set of backends.
    bit_to_backends = {}
    for be, bits in bits_per_backend.items():
        for bit in bits:
            bit_to_backends.setdefault(bit, set()).add(be)

    # Map each canonical backend name to its type header paths.
    all_canonical = set(bits_per_backend.keys())
    type_map = build_type_headers_for_backends(all_canonical, backend_type_headers, bossed_backend_type_headers)

    header_block = """//   GAMBIT: Global and Modular BSM Inference Tool
//   *********************************************
///  \\file
///
///  Compile-time registration of backend types for
///  {bit}.
///
///  This file was automatically generated by
///  backend_harvester.py. Do not modify.
...
"""
    ...
    for bit in sorted(bit_to_backends.keys()):
        bit_regular = set()
        bit_bossed = set()
        for be in bit_to_backends[bit]:
            for h in type_map.get(be, set()):
                if '/' in h:
                    bit_bossed.add(h)
                else:
                    bit_regular.add(h)

        guard = "__backend_types_rollcall_{bit}_hpp__".format(bit=bit)
        towrite = header_block.format(
            bit=bit,
            date=datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y")
        )
        towrite += "\n#ifndef {guard}\n#define {guard}\n".format(guard=guard)
        towrite += "\n#include \"gambit/Utils/type_macros.hpp\"\n"
        towrite += "#include \"gambit/Backends/default_bossed_versions.hpp\"\n"
        if bit_regular:
            towrite += "\n// Backend type definitions for {bit}.\n".format(bit=bit)
            for h in sorted(bit_regular):
                towrite += "#include \"gambit/Backends/backend_types/{0}\"\n".format(h)
        if bit_bossed:
            towrite += "\n// BOSSed backend type definitions for {bit}.\n".format(bit=bit)
            for h in sorted(bit_bossed):
                towrite += "#include \"gambit/Backends/backend_types/{0}\"\n".format(h)
        towrite += "\n#endif // defined {guard}\n".format(guard=guard)

        outpath = "./Backends/include/gambit/Backends/backend_types_rollcall_{bit}.hpp".format(bit=bit)
        with open(outpath, "w") as f:
            f.write(towrite)

    if verbose:
        print("Generated per-module backend type rollcall files.\n")
