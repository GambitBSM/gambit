...
def constrArgsBracket(args, include_arg_name=True, include_arg_type=True, include_namespace=False,
    ...
    for i in range(len(args)):
        ...
        if arg_dict['loaded_class'] and (add_namespace_to_loaded != ''):
            ...
            arg_dict['type_namespaces'] = add_namespaces + \
                arg_dict['type_namespaces']
            arg_dict['type'] = add_namespace_to_loaded + \
                '::' + arg_dict['type']
            ...
            if arg_dict['loaded_class']:
                ...
                if wrapper_to_pointer:
                    if arg_dict['type'].count('*') == 0:
                        ...
                        use_name = '(*' + \
                            arg_dict['name'] + ')' + '.get_BEptr()'
                    args_seq += 'dynamic_cast< ' + \
                        cast_to_type + ' >(' + use_name + ')'
                else:
                    args_seq += 'dynamic_cast< ' + cast_to_type + \
                        ' >(' + arg_dict['name'] + ')'
            ...
            if include_arg_name:
                if utils.isLoadedClass(arg_dict['type'], byname=True) and wrapper_to_pointer:
                    if arg_dict['type'].count('*') == 0:
                        ...
                        raise Exception(
                            'funcutils.constrArgsBracket cannot presently deal with arguments of type pointer-to-pointer for wrapper classes.')
                else:
                    args_seq += arg_dict['name']
...
def constrWrapperName(func_el, include_full_namespace=True):
    ...
    if is_operator:
        w_func_name = 'operator_' + \
            gb.operator_names[func_name_short] + gb.code_suffix
    else:
        ...
        if arg_dict['native'] and not arg_dict['enumeration']:
            if arg_dict['loaded_class']:
                ...
                if convert_loaded_to_abstract:
                    arg_dict['type'] = classutils.toAbstractType(
                        arg_dict['type'])
...
def constrWrapperBody(return_type, func_name, args, return_is_loaded_class, keywords=[]):
    ...
    args_bracket_notypes = constrArgsBracket(
        args, include_arg_type=False, cast_to_original=True)
    ...
    if return_type == 'void':
        ...
        if return_is_loaded_class:
            if is_ref:
                ...
                w_func_body += 'new ' + use_return_type + \
                    '(' + func_name + args_bracket_notypes + ');\n'
        else:
            w_func_body += func_name + args_bracket_notypes + ';\n'
    ...
    if 'returns' in func_el.keys():
        ...
        if not utils.isAcceptedType(return_el):
            if print_warning:
                ...
                infomsg.IgnoredFunction(
                    is_operator*'operator'+func_name['long_templ_args'], reason).printMessage()
            return True
...
def constrExternFuncDecl(func_el):
    ...
    return_type = return_type_dict['name'] + '*' * \
        return_type_dict['pointerness'] + '&'*return_type_dict['is_reference']
...
def getFunctionNameDict(func_el):
    ...
    if 'name' not in func_el.keys():
        raise KeyError(
            'XML element %s does not contain the key "name".' % (xml_id))
    ...
    args_bracket = constrArgsBracket(
        args, include_arg_name=False, include_arg_type=True, include_namespace=True)
    func_name['long_templ_args'] = func_name['long_templ'] + args_bracket
    func_name['short_templ_args'] = func_name['short_templ'] + args_bracket

    # Add return type
    func_name['long_templ_return_args'] = return_type + \
        ' ' + func_name['long_templ_args']
