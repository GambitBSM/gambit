...
#include "gambit/Backends/frontends/SARAHSPheno_Inert2_4_0_3.hpp"
#include "gambit/Elements/spectrum_factories.hpp"
#include "gambit/Models/SimpleSpectra/Inert2SimpleSpec.hpp"
...
BE_NAMESPACE
{
  ...
  {
    throw std::runtime_error("SARAHSPheno_Inert2 backend called TerminateProgram.");
  }
}
END_BE_NAMESPACE
...
BE_NAMESPACE
{
  ...
  namespace Fdecays
  {
    ...
    // Function that reads a table of all the possible decays in SARAHSPheno_Inert2
  ...
  void fill_spectrum_calculate_BRs(const Spectrum &spectrum, const Finputs& inputs)
  {
    ...
    *kont =  0;
    ...
    (*MFe)(3) = sminputs.mTau;
    (*MFe2)(3) = pow((*MFe)(3), 2);
    (*Mhh) = spectrum.get(Par::Pole_Mass, "h0_1");
    (*Mhh2) = pow((*Mhh), 2);
    (*MH0) = spectrum.get(Par::Pole_Mass, "h0_2");
    (*MH02) = pow((*MH0), 2);
    (*MA0) = spectrum.get(Par::Pole_Mass, "A0");
    (*MA02) = pow((*MA0), 2);
    (*MHp)(2) = spectrum.get(Par::Pole_Mass, "H-");
    (*MHp2)(2) = pow((*MHp)(2), 2);
    ...
    *MHD2 = spectrum.get(Par::dimensionless, "MHD2");
    *MHU2 = spectrum.get(Par::dimensionless, "MHU2");
    *v = spectrum.get(Par::dimensionless, "v");
    for(int i=1; i<=2; i++)
    {
      for(int j=1; j<=2; j++)
      {
    ...
    *betaH = asin(abs((*ZP)(1,2)));
    ...
    try{ CalculateBR_2(*CTBD,*ratioWoM,*epsI,*deltaM,*kont,*MA0,*MA02,*MFd,*MFd2,*MFe,*MFe2,*MFu,*MFu2,*MG0,*MG02,*MH0,*MH02,*Mhh,*Mhh2,*MHp,*MHp2,*MVWp,*MVWp2,*MVZ,*MVZ2,*TW,*ZDL,*ZDR,*ZEL,*ZER,*ZP,*ZUL,*ZUR,*ZW,*ZZ,*betaH,*v,*g1,*g2,*g3,*lam5,*lam1,*lam4,*lam3,*lam2,*Ye,*Yd,*Yu,*MHD2,*MHU2,*gPFu,*gTFu,*BRFu,*gPFe,*gTFe,*BRFe,*gPFd,*gTFd,*BRFd,*gPhh,*gThh,*BRhh,*gPH0,*gTH0,*BRH0,*gPA0,*gTA0,*BRA0,*gPHp,*gTHp,*BRHp); }
    catch(std::runtime_error &e) { invalid_point().raise(e.what()); }
  ...
  int run_SPheno_decays(const Spectrum &spectrum, DecayTable& decays, const Finputs& inputs)
  {
    ...
    auto gT1L = [&](int i)
    {
      if(i==1) return (*gT1Lhh);
      else if(i==4) return (*gT1LHp)(i-2);
    ...
    auto gP1L = [&](int i, int j)
    {
      ...
      else if(i==4) return (*gP1LHp)(i-2 ,j);
  ...
  Spectrum Spectrum_Out(const Finputs &inputs)
  {
    ...
    slha["HMIX"][""] << 20 << *MHD2 << "# MHD2";
    slha["HMIX"][""] << 21 << *MHU2 << "# MHU2";
    slha["HMIX"][""] << 3 << *v << "# v";
    ...
    SLHAea_add_block(slha, "HDM", Q);
    SLHAea_add_block(slha, "IMHDM", Q);
    slha["HDM"][""] << 31 << lam1->re << "# Re(lam1)";
    slha["IMHDM"][""] << 31 << lam1->im << "# Im(lam1)";
    slha["HDM"][""] << 32 << lam2->re << "# Re(lam2)";
    slha["IMHDM"][""] << 32 << lam2->im << "# Im(lam2)";
    slha["HDM"][""] << 33 << lam3->re << "# Re(lam3)";
    slha["IMHDM"][""] << 33 << lam3->im << "# Im(lam3)";
    slha["HDM"][""] << 34 << lam4->re << "# Re(lam4)";
    slha["IMHDM"][""] << 34 << lam4->im << "# Im(lam4)";
    slha["HDM"][""] << 35 << lam5->re << "# Re(lam5)";
    slha["IMHDM"][""] << 35 << lam5->im << "# Im(lam5)";
    
    SLHAea_add_block(slha, "MINPAR", Q);
    SLHAea_add_block(slha, "IMMINPAR", Q);
    slha["MINPAR"][""] << 1 << Lambdaa1IN->re << "# Re(Lambdaa1IN)";
    slha["IMMINPAR"][""] << 1 << Lambdaa1IN->im << "# Im(Lambdaa1IN)";
    slha["MINPAR"][""] << 2 << Lambdaa2IN->re << "# Re(Lambdaa2IN)";
    slha["IMMINPAR"][""] << 2 << Lambdaa2IN->im << "# Im(Lambdaa2IN)";
    slha["MINPAR"][""] << 3 << Lambdaa3IN->re << "# Re(Lambdaa3IN)";
    slha["IMMINPAR"][""] << 3 << Lambdaa3IN->im << "# Im(Lambdaa3IN)";
    slha["MINPAR"][""] << 4 << Lambdaa4IN->re << "# Re(Lambdaa4IN)";
    slha["IMMINPAR"][""] << 4 << Lambdaa4IN->im << "# Im(Lambdaa4IN)";
    slha["MINPAR"][""] << 5 << Lambdaa5IN->re << "# Re(Lambdaa5IN)";
    slha["IMMINPAR"][""] << 5 << Lambdaa5IN->im << "# Im(Lambdaa5IN)";
    slha["MINPAR"][""] << 9 << MHUIN->re << "# Re(MHUIN)";
    slha["IMMINPAR"][""] << 9 << MHUIN->im << "# Im(MHUIN)";
    
    SLHAea_add_block(slha, "HDMIN", Q);
    SLHAea_add_block(slha, "IMHDMIN", Q);
    slha["HDMIN"][""] << 35 << lam5->re << "# Re(lam5)";
    slha["IMHDMIN"][""] << 35 << lam5->im << "# Im(lam5)";
    slha["HDMIN"][""] << 31 << lam1->re << "# Re(lam1)";
    slha["IMHDMIN"][""] << 31 << lam1->im << "# Im(lam1)";
    slha["HDMIN"][""] << 34 << lam4->re << "# Re(lam4)";
    slha["IMHDMIN"][""] << 34 << lam4->im << "# Im(lam4)";
    slha["HDMIN"][""] << 33 << lam3->re << "# Re(lam3)";
    slha["IMHDMIN"][""] << 33 << lam3->im << "# Im(lam3)";
    slha["HDMIN"][""] << 32 << lam2->re << "# Re(lam2)";
    slha["IMHDMIN"][""] << 32 << lam2->im << "# Im(lam2)";
    
    SLHAea_add_block(slha, "HMIXIN", Q);
    slha["HMIXIN"][""] << 20 << *MHD2 << "# MHD2";
    slha["HMIXIN"][""] << 21 << *MHU2 << "# MHU2";
    ...
    slha["MASS"][""] << 25 << (*Mhh) << "# h_";
    slha["MASS"][""] << 35 << (*MH0) << "# H0_";
    slha["MASS"][""] << 36 << (*MA0) << "# A0_";
    slha["MASS"][""] << 37 << (*MHp)(2) << "# Hm2_2";
    slha["MASS"][""] << 23 << *MVZ << "# VZ";
    ...
    if(*GetMassUncertainty)
    {
      ...
      slha["DMASS"][""] << 25 << sqrt(pow((*mass_uncertainty_Q)(4),2)+pow((*mass_uncertainty_Yt)(4),2)) << "# h_";
      slha["DMASS"][""] << 35 << sqrt(pow((*mass_uncertainty_Q)(6),2)+pow((*mass_uncertainty_Yt)(6),2)) << "# H0_";
      slha["DMASS"][""] << 36 << sqrt(pow((*mass_uncertainty_Q)(5),2)+pow((*mass_uncertainty_Yt)(5),2)) << "# A0_";
      slha["DMASS"][""] << 37 << sqrt(pow((*mass_uncertainty_Q)(2),2)+pow((*mass_uncertainty_Yt)(2),2)) << "# Hm2_2";
    ...
    Spectrum spectrum = spectrum_from_SLHAea<Gambit::Models::Inert2SimpleSpec, SLHAstruct>(slha,slha,mass_cut,mass_ratio_cut);
  ...
  int get_HiggsCouplingsTable(const Spectrum& spectrum, HiggsCouplingsTable& hctbl, const Finputs& inputs)
  {
    ...
    hctbl.C_WW[0] = (*rHB_S_VWp)(1); // Coupling (h0_1 W+ W-)
    ...
    hctbl.C_gaga2[0] = pow( (*ratioPP).re, 2 ); // Coupling (h0_1 gamma gamma)
    hctbl.C_gg2[0] = pow( (*ratioGG).re, 2 ); // Coupling (h0_1 glu glu)
  ...
  void ReadingData_decays(const Finputs &inputs)
  {
    ...
    *CalcLoopDecay_hh = inputs.options->getValueOrDef<bool>(true, "CalcLoopDecay_hh");
    ...
    // CalcLoopDecay_H0
    *CalcLoopDecay_H0 = inputs.options->getValueOrDef<bool>(true, "CalcLoopDecay_H0");
    
    // CalcLoopDecay_A0
    *CalcLoopDecay_A0 = inputs.options->getValueOrDef<bool>(true, "CalcLoopDecay_A0");
    
    // CalcLoopDecay_Hp
    *CalcLoopDecay_Hp = inputs.options->getValueOrDef<bool>(true, "CalcLoopDecay_Hp");
