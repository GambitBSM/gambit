...
#include "gambit/Backends/frontends/SARAHSPheno_Inert_4_0_3.hpp"
...
BE_NAMESPACE
{
  ...
  {
    throw std::runtime_error("SARAHSPheno_Inert backend called TerminateProgram.");
  }
}
END_BE_NAMESPACE
...
BE_NAMESPACE
{
  ...
  namespace Fdecays
  {
    ...
    // Function that reads a table of all the possible decays in SARAHSPheno_Inert
  ...
  Spectrum Spectrum_Out(const Finputs &inputs)
  {
    ...
    slha["HDM"][""] << 1 << Lam1->re << "# Re(Lam1)";
    slha["IMHDM"][""] << 1 << Lam1->im << "# Im(Lam1)";
    slha["HDM"][""] << 2 << Lam2->re << "# Re(Lam2)";
    slha["IMHDM"][""] << 2 << Lam2->im << "# Im(Lam2)";
    slha["HDM"][""] << 3 << Lam3->re << "# Re(Lam3)";
    slha["IMHDM"][""] << 3 << Lam3->im << "# Im(Lam3)";
    slha["HDM"][""] << 4 << Lam4->re << "# Re(Lam4)";
    slha["IMHDM"][""] << 4 << Lam4->im << "# Im(Lam4)";
    slha["HDM"][""] << 5 << Lam5->re << "# Re(Lam5)";
    slha["IMHDM"][""] << 5 << Lam5->im << "# Im(Lam5)";
    
    SLHAea_add_block(slha, "MINPAR", Q);
    SLHAea_add_block(slha, "IMMINPAR", Q);
    slha["MINPAR"][""] << 1 << Lambda1IN->re << "# Re(Lambda1IN)";
    slha["IMMINPAR"][""] << 1 << Lambda1IN->im << "# Im(Lambda1IN)";
    slha["MINPAR"][""] << 2 << Lambda2IN->re << "# Re(Lambda2IN)";
    slha["IMMINPAR"][""] << 2 << Lambda2IN->im << "# Im(Lambda2IN)";
    slha["MINPAR"][""] << 3 << Lambda3IN->re << "# Re(Lambda3IN)";
    slha["IMMINPAR"][""] << 3 << Lambda3IN->im << "# Im(Lambda3IN)";
    slha["MINPAR"][""] << 4 << Lambda4IN->re << "# Re(Lambda4IN)";
    slha["IMMINPAR"][""] << 4 << Lambda4IN->im << "# Im(Lambda4IN)";
    slha["MINPAR"][""] << 5 << Lambda5IN->re << "# Re(Lambda5IN)";
    slha["IMMINPAR"][""] << 5 << Lambda5IN->im << "# Im(Lambda5IN)";
    
    SLHAea_add_block(slha, "HDMIN", Q);
    SLHAea_add_block(slha, "IMHDMIN", Q);
    slha["HDMIN"][""] << 5 << Lam5->re << "# Re(Lam5)";
    slha["IMHDMIN"][""] << 5 << Lam5->im << "# Im(Lam5)";
    slha["HDMIN"][""] << 1 << Lam1->re << "# Re(Lam1)";
    slha["IMHDMIN"][""] << 1 << Lam1->im << "# Im(Lam1)";
    slha["HDMIN"][""] << 4 << Lam4->re << "# Re(Lam4)";
    slha["IMHDMIN"][""] << 4 << Lam4->im << "# Im(Lam4)";
    slha["HDMIN"][""] << 3 << Lam3->re << "# Re(Lam3)";
    slha["IMHDMIN"][""] << 3 << Lam3->im << "# Im(Lam3)";
    slha["HDMIN"][""] << 2 << Lam2->re << "# Re(Lam2)";
    slha["IMHDMIN"][""] << 2 << Lam2->im << "# Im(Lam2)";
    
    SLHAea_add_block(slha, "MSOFTIN", Q);
    slha["MSOFTIN"][""] << 21 << *mHd2 << "# mHd2";
    slha["MSOFTIN"][""] << 22 << *mHu2 << "# mHu2";
    ...
    slha["MASS"][""] << 25 << (*Mhh) << "# h_";
    slha["MASS"][""] << 35 << *MH0 << "# H0_0";
    slha["MASS"][""] << 36 << *MA0 << "# A0_0";
    slha["MASS"][""] << 37 << (*MHp)(2) << "# Hp2_2";
    slha["MASS"][""] << 23 << *MVZ << "# VZ";
    ...
    if(*GetMassUncertainty)
    {
      ...
      slha["DMASS"][""] << 35 << sqrt(pow((*mass_uncertainty_Q)(6),2)+pow((*mass_uncertainty_Yt)(6),2)) << "# H0_0";
      slha["DMASS"][""] << 36 << sqrt(pow((*mass_uncertainty_Q)(5),2)+pow((*mass_uncertainty_Yt)(5),2)) << "# A0_0";
      slha["DMASS"][""] << 37 << sqrt(pow((*mass_uncertainty_Q)(2),2)+pow((*mass_uncertainty_Yt)(2),2)) << "# Hp2_2";
    ...
    Spectrum spectrum = spectrum_from_SLHAea<Gambit::Models::InertSimpleSpec, SLHAstruct>(slha,slha,mass_cut,mass_ratio_cut);
  ...
  int get_HiggsCouplingsTable(const Spectrum& spectrum, HiggsCouplingsTable& hctbl, const Finputs& inputs)
  {
    ...
    hctbl.C_WW2[0] = (*rHB_S_VWp)(1); // Coupling (h0_1 W+ W-)
    hctbl.C_ZZ2[0] = (*rHB_S_VZ)(1); // Coupling (h0_1 Z Z)
