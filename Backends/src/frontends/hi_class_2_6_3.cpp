...
///  \file
///
///  Frontend source for the classy backend. TODO: Alter so it isnt just a copy of class one
///
///  *********************************************
///
...
#include "gambit/Backends/frontends/hi_class_2_6_3.hpp"
  ...
  BE_NAMESPACE
  {
    ...
    pybind11::object get_hi_class_cosmo_object()
    {
      return cosmo;
    }

    // return backend directory of CLASS
    std::string get_hi_class_backendDir()
    {
      return backendDir;
    }
    ...
    bool compare_dicts(pybind11::dict hi_class_input, pybind11::dict prev_input_dict)
    {
      for (auto it : hi_class_input)
      {
         ...
         if (hi_class_input[it.first].cast<std::string>() != prev_input_dict[it.first].cast<std::string>()){return false;}
      }
    ...
    void hi_class_input_consistency_checks(pybind11::dict hi_class_input)
    {
      ...
      if(hi_class_input.contains("modes") and not hi_class_input.contains("output"))
      {
        errMssg << "You are calling CLASS asking for the following modes to be computed : "<< pybind11::repr(hi_class_input["modes"]);
    ...
    // get the lensed Cl.
    std::vector<double> hi_class_get_lensed_cl(std::string spectype)
    {
    ...
    std::vector<double> hi_class_get_unlensed_cl(std::string spectype)
    {
    ...
    double hi_class_get_Da(double z)
    {
    ...
    double hi_class_get_Dl(double z)
    {
    ...
    double hi_class_get_scale_independent_growth_factor(double z)
    {
    ...
    double hi_class_get_scale_independent_growth_factor_f(double z)
    {
    ...
    double hi_class_get_Hz(double z)
    {
    ...
    double hi_class_get_Omega0_ncdm_tot()
    {
    ...
    double hi_class_get_Omega0_Lambda()
    {
    ...
    void print_pps()
    {
      std::cout<< "Primordial spectrum from hi_class: "<< std::string(pybind11::str(cosmo.attr("get_primordial")())) << std::endl;
    }
...
BE_INI_FUNCTION
{
    ...
    Classy_input input_container = *Dep::hi_class_input_params;
    ...
    if (cosmo_input_dict.attr("__contains__")("f_eff_type").cast<bool>())
    {
      ...
      error << "\n\ncd build; make nuke-hi_class_"<< STRINGIFY(VERSION) <<"; make hi_class; cd ..";
    ...
    if(first_run)
    {
      ...
      cosmo = hi_class.attr("Class")();
      // check input for consistency
      hi_class_input_consistency_checks(cosmo_input_dict);
    ...
    if(not equal or first_run)
    {
      ...
      logger() << LogTags::debug << "[hi_class_"<< STRINGIFY(VERSION) <<"] These are the inputs:"<<endl;
      ...
      logger() << LogTags::info << "[hi_class_"<< STRINGIFY(VERSION) <<"] Start to run \"cosmo.compute\"" << EOM;
      try
      {
        // Try to run hi_class
        cosmo.attr("compute")();
        ...
        error_counter = 0;
        logger() << LogTags::info << "[hi_class_"<< STRINGIFY(VERSION) <<"] \"cosmo.compute\" was successful" << EOM;
      }
      catch (std::exception &e)
      {
        std::ostringstream errMssg;
        errMssg << "Could not successfully execute cosmo.compute() in hi_class_"<< STRINGIFY(VERSION)<<"\n";
        ...
        else
        {
          ...
          errMssg << rawErrMessage;
          cout << "An unspecified error occurred during compute() in hi_class_"<< STRINGIFY(VERSION) <<":\n";
          cout << rawErrMessage;
    ...
    {
      logger() << LogTags::info << "[hi_class_"<< STRINGIFY(VERSION) <<"] \"cosmo.compute\" was skipped, input was identical to previously computed point" << EOM;
