...
///  \file
///
///  ScannerBit interface to Multinest 3.13
///
///  *********************************************
///
...
#include "gambit/ScannerBit/scanners/multinest_3.13/multinest.hpp"
...
scanner_plugin(multinest, version(3, 13))
{
   ...
   int plugin_main (void)
   {
      ...
      // Get the MPI communicator as a Fortran integer handle
      // Negative value tells MultiNest to use MPI_COMM_WORLD
      int mpi_comm = -1;
#ifdef WITH_MPI
      mpi_comm = MPI_Comm_c2f(get_mpi_comm());
#endif
      ...
      Gambit::MultiNest_3_13::LogLikeWrapper loglwrapper(LogLike, get_printer());
      Gambit::MultiNest_3_13::global_loglike_object = &loglwrapper;

      //Run MultiNest, passing callback functions for the loglike and dumper.
      if(myrank == 0) std::cout << "Starting MultiNest run..." << std::endl;
      run_mpi(IS, mmodal, ceff, nlive, tol, efr, ndims, nPar, nClsPar, maxModes, updInt, Ztol,
          root, seed, &pWrap[0], fb, resume, outfile, initMPI, ln0, maxiter,
          Gambit::MultiNest_3_13::callback_loglike, Gambit::MultiNest_3_13::callback_dumper, context, mpi_comm);
