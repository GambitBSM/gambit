...
double LoopFunctions::C3q(char q, double muW, double Lambda, double Ctil_e, double Ctil_mu, double Ctil_tau, double Ctil_u, double Ctil_d, double Ctil_s, double Ctil_c, double Ctil_b, double Ctil_t){
    ...
    double res = std::pow(vev,3)/(std::sqrt(2) * std::pow(Lambda,2))*(
         + 3.*aMZ*as51lMh/(4.*pi*pi) * 1/mt * Ctil_t * (Qu*Qu*xth*DTPHI(1./(4*xth)) + Qu/(16.*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2*Qu)*xth*xtz/(xtz-xth)*(DTPHI(1/(4*xtz)) - DTPHI(1/(4*xth))))
         + aMZ*as51lMh/(3*pi*pi) * 1/mq * Ctilq * (Qu*Qu*xth*((1-2*xth)*DTPHI(1/(4*xth)) + 2*(2+std::log(xth))) + Qu/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2*Qu)*xth*xtz/(xtz-xth)*((1-2*xth)*DTPHI(1/(4*xth)) - 2*std::log(xhz) - (1-2*xtz)*DTPHI(1/(4*xtz))))
         + 3*aMZ*as51lMh/(4*pi*pi) * 1/mb * Ctil_b * (Qd*Qd*xbh*(std::pow(std::log(xbh),2) + pi*pi/3.) + Qd/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1+4*sw2*Qd)*xbh*xbz/(xbz-xbh)*(std::pow(std::log(xbh),2) - std::pow(std::log(xbz),2)))
         + 3*aMZ*as51lMh/(4*pi*pi) * 1/mc * Ctil_c * (Qu*Qu*xch*(std::pow(std::log(xch),2) + pi*pi/3.) + Qu/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2*Qu)*xch*xcz/(xcz-xch)*(std::pow(std::log(xch),2) - std::pow(std::log(xcz),2)))
         + 1*aMZ*as51lMh/(4*pi*pi) * 1/mtau * Ctil_tau * (xtauh*(std::pow(std::log(xtauh),2) + pi*pi/3.) - 1/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2)*xtauh*xtauz/(xtauz-xtauh)*(std::pow(std::log(xtauh),2) - std::pow(std::log(xtauz),2)))
         + 1*aMZ*as51lMh/(4*pi*pi) * 1/mmu * Ctil_mu * (xmuh*(std::pow(std::log(xmuh),2) + pi*pi/3.) - 1/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2)*xmuh*xmuz/(xmuz-xmuh)*(std::pow(std::log(xmuh),2) - std::pow(std::log(xmuz),2)))
         + 1*aMZ*as51lMh/(4*pi*pi) * 1/me * Ctil_e * (xeh*(std::pow(std::log(xeh),2) + pi*pi/3.) - 1/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2)*xeh*xez/(xez-xeh)*(std::pow(std::log(xeh),2) - std::pow(std::log(xez),2)))
...
double LoopFunctions::C3e(char l, double muW, double Lambda, double Ctil_e, double Ctil_mu, double Ctil_tau, double Ctil_u, double Ctil_d, double Ctil_s, double Ctil_c, double Ctil_b, double Ctil_t){
    ...
    double ml = 0.; double Ctill = 0.;
    double mubarrzee = 1; //if l=mu no barr zee with muon loop
    if (l=='e'){ ml = me; Ctill = Ctil_e;}
    if (l=='m'){ ml = mmu; Ctill = Ctil_mu; mubarrzee = 0.;}
    double res = 1/(4*pi)*std::pow(vev,3)/(std::sqrt(2) * std::pow(Lambda,2))*(
         + 3.*aMZ/(4.*pi*pi) * 1/mt * Ctil_t * (Qu*Qu*xth*DTPHI(1./(4*xth)) + Qu/(16.*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2*Qu)*xth*xtz/(xtz-xth)*(DTPHI(1/(4*xtz)) - DTPHI(1/(4*xth))))
         + aMZ/(3*pi*pi) * 1/ml * Ctill * (
         ...
         + 3*aMZ/(4*pi*pi) * 1/mb * Ctil_b * (Qd*Qd*xbh*(std::pow(std::log(xbh),2) + pi*pi/3.) + Qd/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1+4*sw2*Qd)*xbh*xbz/(xbz-xbh)*(std::pow(std::log(xbh),2) - std::pow(std::log(xbz),2)))
         + 3*aMZ/(4*pi*pi) * 1/mc * Ctil_c * (Qu*Qu*xch*(std::pow(std::log(xch),2) + pi*pi/3.) + Qu/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2*Qu)*xch*xcz/(xcz-xch)*(std::pow(std::log(xch),2) - std::pow(std::log(xcz),2)))
         + 1*aMZ/(4*pi*pi) * 1/mtau * Ctil_tau * (xtauh*(std::pow(std::log(xtauh),2) + pi*pi/3.) - 1/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2)*xtauh*xtauz/(xtauz-xtauh)*(std::pow(std::log(xtauh),2) - std::pow(std::log(xtauz),2)))
         + 1*aMZ/(4*pi*pi) * 1/mmu * Ctil_mu * (xmuh*(std::pow(std::log(xmuh),2) + pi*pi/3.) - 1/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2)*xmuh*xmuz/(xmuz-xmuh)*(std::pow(std::log(xmuh),2) - std::pow(std::log(xmuz),2))) * mubarrzee
         - aMZ/(32*pi*pi) * 1/ml * 3./(2*cw2) * Ctill * std::log(std::pow(muW/Lambda,2))
        );

    std::cout << "lf.C3e: " << l << ": " << res << std::endl;
    std::cout << "lf.C3e.Ctill: " << l << ": " << Ctill << std::endl;
    std::cout << "lf.C3e.BZ: " << + aMZ/(6*pi*pi) * 1/ml * Ctill * (
            + Qu*Qu*xth*((1-2*xth)*DTPHI(1./(4*xth)) + 2*(2+std::log(xth)))
            + Qu/(16*Qq*cw2*sw2)*(1+pq*4*sw2*Qq)*(1-4*sw2*Qu)*xth*xtz/(xtz-xth)*((1-2*xth)*DTPHI(1./(4*xth)) - 2*std::log(xhz) - (1-2*xtz)*DTPHI(1./(4*xtz)))) << std::endl;
    std::cout << "lf.C3e.log: " << - aMZ/(32*pi*pi) * 1/ml * 3./(2*cw2) * Ctill * std::log(std::pow(muW/Lambda,2)) << std::endl;
...
double LoopFunctions::C4q(char q, double muW, double Lambda, double Ctil_u, double Ctil_d, double Ctil_s, double Ctil_t){
	...
	+ as51lMh*aMZ/(576*pi*pi*cw2*sw2) * 1/mq * Ctilq * (1+pm*4*Qq*sw2 + 8*Qq*Qq*sw2*sw2)*xzh*xzh*
		(
	...
	+ as51lMh*aMZ/(288*pi*pi*sw2) * 1/mq * Ctilq * xhw*
		(
...
double LoopFunctions::Cw(double Lambda, double Ctil_t){
    double res = std::pow(vev,3)/(std::sqrt(2) * std::pow(Lambda,2))*(
        + as51lMh*as51lMh/(32*pi*pi) * 1/mt * Ctil_t * xth/std::pow((1-4*xth),3) * (3-14*xth+8*xth*xth + 6*xth*(1-2*xth+2*xth*xth)*DTPHI(1/(4*xth)) + (2+10*xth - 12*xth*xth)*std::log(xth))
        );
    return res;
}
...
double LoopFunctions::C1qqp(char q, char qp, double Lambda, double Ctil_u, double Ctil_d, double Ctil_s, double Ctil_c, double Ctil_b){
    ...
    return 1/(Mh*Mh) * std::pow(vev,1)/(std::sqrt(2) * std::pow(Lambda,2)) * mq * Ctilqp;
}
...
double LoopFunctions::C1q(char q, double Lambda, double Ctil_u, double Ctil_d, double Ctil_s, double Ctil_c, double Ctil_b){
    ...
    return 1/(Mh*Mh) * std::pow(vev,1)/(std::sqrt(2) * std::pow(Lambda,2)) * mq * Ctilq;
}
