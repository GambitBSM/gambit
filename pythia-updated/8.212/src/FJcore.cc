...
// PJE: currently appears to be main computational cost in profiling
// calls a lot of inline functions so to get better handle on 
// profiling might be easiest to convert these inline functions 
// to just functions 
// Also question as to why this function is a templated class 
// not all classes will have the properties to make this function 
// compile and work. Surely there is a parent class called 
// Jets somewhere that all classes could inherit from? 
// Is the idea behind the use of templates to just get around 
// inheritance and having to cast child class objects as parent
// class objects? 

// PJE: Ideas going forward. Force use _simple_N2_cluster
// and then see about perhaps generating a dist array to do 
// all the calculations at the beginning and not do it within a loop
// also might be useful to have an NN array rather than a linked list. 
// Suggested updates will require altering the Jet class
// I may also alter the loops to be easier to vectorise 
// and also add pragma simd when possible

template<class BJ> void ClusterSequence::_simple_N2_cluster() {
  // PJE: More profiling need to be done
  // need to check computational cost of 
  // _bj_set_jetinfo 
  ...
  // _bj_set_NN_nocross
  // these are in FJcore.hh file 
  /*

  // jet info, inline function 
  // computation could be J::rap(), J::phi_02pi(), 
  // and the jet_scale_for_algorithm();
  // Am a bit puzzled by the use of a template of class J
  // since not all classes will have properties eta, phi, kt2
  // Why template it? Surely all Jets have a general parent class
  // 
...
// bjet distance calculation
// not much computation since it just has std::abs and +,-, * operators
...
// Again, little compute since it is just an if statment with assignment 
...
// loop but not much computation since calls just djet distance calculation
// which doesn't have much compute 
...
//Similar to above in computational cost
template <class J> inline void ClusterSequence::_bj_set_NN_crosscheck(J * const jet,
  ...
  */
  ...
  // PJE: allocate BJ jet class array
  // use jetA and jetB pointers to point to it 
  // and set properties
  int n = _jets.size();
  BJ * briefjets = new BJ[n];
  BJ * jetA = briefjets, * jetB;
  // PJE: this is an odd loop. Why not have a 
  //vector of BJ and just iterate over it? 
  // vector<BJ> briefjets(n); for (auto &bg:briefjets) _bj_set_jetinfo(&bf,i);
  for (int i = 0; i< n; i++) {
    _bj_set_jetinfo(jetA, i);
    jetA++; // move on to next entry of briefjets
  }

  // PJE: here again it uses pointers to go through array. Why?
  // is it just to use the pointer increment operator?
  // loop appears to just be 
  // for (auto i=1;i<n-1;i++) _bj_set_NN_crosscheck(&BJ[i],&BJ[0],&BJ[i]);
  ...
  // PJE: again, any reason to have this type of loop
  // and this allocation. What does _bj_diJ do?
  // vector<double> diJ(n);
  // for (auto i=0;i<n;i++) diJ[n] = _bi_diJ(&BJ[i]);
  ...
  // history starts at the end (or tail)
  int history_location = n-1;
  // go through into tail and head 

  while (tail != head) {
    // get the minimum jet value value and index
...
/// GPU enabled simple jet clustering
#ifdef USEGPU
template<class BJ> void ClusterSequence::_simple_N2_cluster_GPU() {
  ...
  // and set properties
  int n = _jets.size();
  vector<BJ> briefjets(n), briefjets_NN(n);
  BJ * head = &briefjet[0], tail = &briefjet[n-1];
  vector<double> diJ(n);
  n=0;
  // loop over the briefjets and initialise
  if (n > FJCORE_OMPGPUOFFLOAD) {
    ...
    if (nthreads > 1) 
    (auto i=0;i<n;i++) _bj_set_jetinfo(briefjets[i], i);
  }
  ...
  for (auto i=1;i<n;i++) _bj_set_NN_crosscheck(&briefjet[i], head, &briefjet[i]);
  ...
  if (n > FJCORE_OMPGPUOFFLOAD) {
    ...
    num_threads(nthreads) \
    if (nthreads > 1) 
    for (auto i=0;i<n;i++) diJ[i] = _bi_diJ(briefjet[i]); 
  }
...
#ifdef USEGPU
  // if GPU used, only allow GPU N^2 calculation optimised for GPU offloading
  this->_simple_N2_cluster_BriefJet_GPU();
#else 
...
#ifdef USEGPU
void ClusterSequence::_simple_N2_cluster_BriefJet_GPU() {
  _simple_N2_cluster_GPU<BriefJet>();
}
void ClusterSequence::_simple_N2_cluster_EEBriefJet_GPU() {
  _simple_N2_cluster_GPU<EEBriefJet>();
}
#endif
...
// PJE: first function within the jet cluster functions 
// sets some ClusterSequence variables 
void ClusterSequence::_initialise_tiles() {
  ...
  // PJE: now having determined the boundaries of eta and vector size
  // allocate memory 
  _tiles.resize((_tiles_ieta_max-_tiles_ieta_min+1)*_n_tiles_phi);
  // PJE: loop over vector. Seems weird to use pointers 
  for (int ieta = _tiles_ieta_min; ieta <= _tiles_ieta_max; ieta++) {
    for (int iphi = 0; iphi < _n_tiles_phi; iphi++) {
      ...
      // PJE: if ieta is abouve ieta do some extra compute 
      // weird do this since ieta > for all iterations save the first 
      ...
      // PJE: use pointers to access _tiles vector and move 
      // up one (why?)
      *pptile = & _tiles[_tile_index(ieta,iphi-1)];
      pptile++;
      // PJE: set the RH tile. 
      // Could just have _tiles[_tile_index(ieta,iphi-1)+1].RH_tiles = _tiles[_tile_index(ieta,iphi-1)]
      tile->RH_tiles = pptile;
      // PJE similar to above 
      *pptile = & _tiles[_tile_index(ieta,iphi+1)];
      pptile++;
...
// PJE: start exploring the faster_tilded_N2_cluster 
// have removed inline of functions
void ClusterSequence::_faster_tiled_N2_cluster() {
  // PJE: initialise tiles in angular system 
  _initialise_tiles();
  int n = _jets.size();
  // PJE: now allocate array of tiledjet
  TiledJet * briefjets = new TiledJet[n];

  TiledJet * jetA = briefjets, * jetB;
  TiledJet oldB;
  // PJE: why is tile_index not set to zero generating a warning 
  // that needs to be dealt with 
  oldB.tile_index=0; // prevents a gcc warning
  // PJE: set tiled jet info 
  vector<int> tile_union(3*n_tile_neighbours);
  for (int i = 0; i< n; i++) {
    _tj_set_jetinfo(jetA, i);
    jetA++; // move on to next entry of briefjets
  ...
  TiledJet * head = briefjets; // a nicer way of naming start
  vector<Tile>::const_iterator tile;
  // PJE: this triple loop is written in a fashion that goes over all 
  // tiles and all the N*N-1 jet pairs in that tile 
  // loop structure less than ideal. Why is third loop 
  // for jetB != jetA? does that mean that one does 
  // nothing for the first pass where jetA = head and jetB = head? 
  // then the next there is one loop? Poorly written 
  for (tile = _tiles.begin(); tile != _tiles.end(); tile++) {
    for (jetA = tile->head; jetA != NULL; jetA = jetA->next) {
      for (jetB = tile->head; jetB != jetA; jetB = jetB->next) {
        // PJE: calculate distances between jetA and jetB
      	double dist = _bj_dist(jetA,jetB);
    ...
    // PJE: now for all RH tiels of the current tille do the same loop 
    // seems like a lot of cache misses. Why do the RH_tiles separately? 
    for (Tile ** RTile = tile->RH_tiles; RTile != tile->end_tiles; RTile++) {
      for (jetA = tile->head; jetA != NULL; jetA = jetA->next) {
      	for (jetB = (*RTile)->head; jetB != NULL; jetB = jetB->next) {
	        double dist = _bj_dist(jetA,jetB);
  ...
  // PJE: struct that stores distance and the Tiled jet at its minimum distance? 
  struct diJ_plus_link {
    double     diJ; // the distance
    TiledJet * jet; // the jet (i) for which we've found this distance
  };
  // PJE : allocate memory of struct
  diJ_plus_link * diJ = new diJ_plus_link[n];
  // PJE: start with jet = the head of the TiledJet briefjet array 
  jetA = head;
  // PJE: another odd loop where pointer is used to go over array 
  for (int i = 0; i < n; i++) {
  ...
  // PJE: Similar to _simple_N2_cluster while loop
  int history_location = n-1;
  while (n > 0) {
    ...
    if (jetB != NULL) {
      ...
      int nn; // new jet index
      _do_ij_recombination_step(jetA->_jets_index, jetB->_jets_index, diJ_min, nn);
      // PJE: extra remove from tiles call as jetA has been combined 
      _bj_remove_from_tiles(jetA);
      oldB = * jetB;  // take a copy because we will need it...
      // PJE: remove jet that is pointed to by jetB
      _bj_remove_from_tiles(jetB);
      // PJE: but then why set info of jetB
    ...
    int n_near_tiles = 0;
    _add_untagged_neighbours_to_tile_union(jetA->tile_index, tile_union, n_near_tiles);
    if (jetB != NULL) {
      if (jetB->tile_index != jetA->tile_index) {
        _add_untagged_neighbours_to_tile_union(jetB->tile_index, tile_union,n_near_tiles);
      }
      if (oldB.tile_index != jetA->tile_index && oldB.tile_index != jetB->tile_index) {
        _add_untagged_neighbours_to_tile_union(oldB.tile_index, tile_union,n_near_tiles);
      }
    }
    n--;
    diJ[n].jet->diJ_posn = jetA->diJ_posn;
    diJ[jetA->diJ_posn] = diJ[n];
    // PJE: go over the tiels 
    for (int itile = 0; itile < n_near_tiles; itile++) {
      Tile * tile_ptr = &_tiles[tile_union[itile]];
