...
public:
  ...
  // PJE: why a class definition inside a class ? 
  // purely for encapsulation?
  // class is a virutal class but how would you drive from it
  // if declared inside another class?
  class Extras {
  public:
    virtual ~Extras() {}
  ...
  // PJE: this is a weird public statement because scope is 
  // still public 
...
// PJE: private variables
private:
  static std::ostream * _fastjet_banner_ostr;
protected:
  // PJE: some protected varaibles 
  JetDefinition _jet_def;
  // PJE: protected functions 
 ...
 private:
  ...
  void _extract_tree_parents (int pos, std::valarray<bool> &,
                const std::valarray<int> &,  std::vector<int> &) const;
  // PJE: odd to have internal typedefs but not too weird
  // issue is that these are unused. 
  ...
  // PJE: internal brief jet struct in protected scope
  // but why have definition but no instance? 
  struct BriefJet {
    double     eta, phi, kt2, NN_dist;
    BriefJet * NN;
    int        _jets_index;
  };
  // PJE: internal tiled jet struct but again no instance. Why?
  class TiledJet {
  public:
    double     eta, phi, kt2, NN_dist;
  ...
  // PJE: weird that the function is defined here when all others
  // which can be quite small are defined in the .cc
  template <class J> inline J * _bj_of_hindex(
    const int hist_index,
    J * const head, J * const tail) const 
  {
    J * res;
  ...
  // PJE: private structure, has instances 
  struct Tile {
  ...
  // PJE: some more private functions
  void _simple_N2_cluster_BriefJet();
  void _simple_N2_cluster_EEBriefJet();
};
//end of cluster sequence class 

/// \name ClusterSequene Functions 
//@{
