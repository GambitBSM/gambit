...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSTHDM_IIMessage", 1);
...
class THDM_II_spectrum {
public:
   virtual ~THDM_II_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(THDM_II_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const THDM_II_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_II_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class THDM_II_spectrum_impl : public THDM_II_spectrum
{
public:
   virtual ~THDM_II_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(THDM_II_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const THDM_II_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_II_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<THDM_II_slha<THDM_II<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   THDM_II_scales scales{};              ///< scale information
   THDM_II_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const THDM_II_input_parameters& input_) { input = input_; }
...
private:
   THDM_II_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<THDM_II_spectrum> spectrum{nullptr};  ///< spectrum information

   THDM_II_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, THDM_II_info::particle_names[i].c_str());
      }
   }
}
...
THDM_II_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
...
void put_spectrum(const THDM_II_slha<Model>& model, MLINK link)
{
   MLPutRule(link, THDM_II_info::model_name);
...
template <typename Solver_type>
void THDM_II_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void THDM_II_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const THDM_II_input_parameters& input
)
{
...
template <typename Solver_type>
void THDM_II_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void THDM_II_spectrum_impl<Solver_type>::fill_slha_io(THDM_II_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   THDM_II_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSTHDM_IICalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSTHDM_IICalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new THDM_II_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSTHDM_IIGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IIGetSettings"))
...
DLLEXPORT int FSTHDM_IIGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IIGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IIGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IIGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IIOpenHandle(
...
DLLEXPORT int FSTHDM_IICloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::THDM_II_librarylink;
...
DLLEXPORT int FSTHDM_IIGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IIGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IIToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IIToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IIGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IIGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IICalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IICalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IICalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_II_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IICalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSTHDM_IICalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSTHDM_IICalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSTHDM_IICalculateObservables", "error", e.what());
      put_error_output(link);
   }
