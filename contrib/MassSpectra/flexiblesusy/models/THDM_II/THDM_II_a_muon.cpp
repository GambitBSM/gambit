...
// File generated at Thu 2 Aug 2018 15:06:49

/**
 * @file THDM_II_a_muon.cpp
 *
 * This file was generated at Thu 2 Aug 2018 15:06:49 with FlexibleSUSY
...
#include "THDM_II_cxx_diagrams.hpp"
...
double get_MSUSY(const THDM_II_mass_eigenstates& model)
{
   return Min(model.get_MHm().tail<1>().minCoeff());

}

void run_to_MSUSY(THDM_II_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("THDM_II_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("THDM_II_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("THDM_II_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("THDM_II_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(THDM_II_mass_eigenstates& model)
{
   VERBOSE_MSG("THDM_II_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const THDM_II_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("THDM_II_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double THDM_II_a_muon::calculate_a_muon(const THDM_II_mass_eigenstates& model_)
{
   THDM_II_mass_eigenstates model(model_);

   VERBOSE_MSG("THDM_II_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDM_II_a_muon:" << e.what());
...
double THDM_II_a_muon::calculate_a_muon_uncertainty(const THDM_II_mass_eigenstates& model_)
{
   THDM_II_mass_eigenstates model(model_);

   VERBOSE_MSG("THDM_II_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDM_II_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
