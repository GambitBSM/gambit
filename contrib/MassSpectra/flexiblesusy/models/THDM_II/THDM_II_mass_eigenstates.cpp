...
// File generated at Tue 31 Jul 2018 21:13:44
 ...
 * parameters.
 *
 * This file was generated at Tue 31 Jul 2018 21:13:44 with FlexibleSUSY
 * 2.0.1 (git commit: unknown) and SARAH 4.12.3 .
 */

#include "THDM_II_mass_eigenstates.hpp"
#include "THDM_II_ewsb_solver_interface.hpp"
...
CLASSNAME::THDM_II_mass_eigenstates(const THDM_II_input_parameters& input_)
   ...
   , ewsb_solver(new THDM_II_ewsb_solver<Two_scale>())
#endif
{
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<THDM_II_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("THDM_II_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("THDM_II_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("THDM_II_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving THDM_II EWSB at " << ewsb_loop_order << "-loop order");
...
{
   if (PHYSICAL(Mhh).tail<2>().minCoeff() < 0.) problems.flag_pole_tachyon(THDM_II_info::hh);
   if (PHYSICAL(MAh).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(THDM_II_info::Ah);
   if (PHYSICAL(MHm).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(THDM_II_info::Hm);
...
void CLASSNAME::calculate_Mhh()
{
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_running_tachyon(THDM_II_info::hh);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   if (MAh.minCoeff() < 0.) {
      problems.flag_running_tachyon(THDM_II_info::Ah);
   }
...
void CLASSNAME::calculate_MHm()
{
   ...
   if (MHm.minCoeff() < 0.) {
      problems.flag_running_tachyon(THDM_II_info::Hm);
   }
...
void CLASSNAME::calculate_MVWm()
{
   ...
   if (MVWm < 0.) {
      problems.flag_running_tachyon(THDM_II_info::VWm);
   }
...
{
   if (!force_output && problems.is_running_tachyon(THDM_II_info::VZ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(THDM_II_info::VZ);
...
{
   if (!force_output && problems.is_running_tachyon(THDM_II_info::hh))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(THDM_II_info::Ah))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(THDM_II_info::Hm))
      return;
...
{
   if (!force_output && problems.is_running_tachyon(THDM_II_info::VWm))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(THDM_II_info::VWm);
