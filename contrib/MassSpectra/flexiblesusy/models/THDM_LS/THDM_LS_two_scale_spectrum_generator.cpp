...
#include "THDM_LS_two_scale_convergence_tester.hpp"
#include "THDM_LS_two_scale_ewsb_solver.hpp"
#include "THDM_LS_two_scale_initial_guesser.hpp"
...
double THDM_LS_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void THDM_LS_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const THDM_LS_input_parameters& input)
{
   ...
   THDM_LS_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<THDM_LS_ewsb_solver<Two_scale> >(ewsb_solver));

   THDM_LS_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   THDM_LS_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   THDM_LS_convergence_tester<Two_scale> convergence_tester(
   ...
   THDM_LS_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void THDM_LS_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   THDM_LS_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void THDM_LS_spectrum_generator<Two_scale>::calculate_spectrum()
{
