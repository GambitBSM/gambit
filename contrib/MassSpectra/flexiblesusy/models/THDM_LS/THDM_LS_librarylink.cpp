...
// File generated at Tue 7 Apr 2020 14:57:49

#include "config.h"

#include "THDM_LS_info.hpp"
#include "THDM_LS_input_parameters.hpp"
#include "THDM_LS_observables.hpp"
#include "THDM_LS_physical.hpp"
#include "THDM_LS_slha_io.hpp"
#include "THDM_LS_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "THDM_LS_two_scale_model.hpp"
#include "THDM_LS_two_scale_spectrum_generator.hpp"
#endif
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSTHDM_LSMessage", 1);
...
class THDM_LS_spectrum {
public:
   virtual ~THDM_LS_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(THDM_LS_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const THDM_LS_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_LS_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class THDM_LS_spectrum_impl : public THDM_LS_spectrum
{
public:
   virtual ~THDM_LS_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(THDM_LS_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const THDM_LS_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_LS_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<THDM_LS_slha<THDM_LS<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   THDM_LS_scales scales{};              ///< scale information
   THDM_LS_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const THDM_LS_input_parameters& input_) { input = input_; }
...
private:
   THDM_LS_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<THDM_LS_spectrum> spectrum{nullptr};  ///< spectrum information

   THDM_LS_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, THDM_LS_info::particle_names[i].c_str());
      }
   }
}
...
THDM_LS_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   THDM_LS_slha_io slha_io;
...
void put_spectrum(const THDM_LS_slha<Model>& model, MLINK link)
{
   MLPutRule(link, THDM_LS_info::model_name);
...
template <typename Solver_type>
void THDM_LS_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void THDM_LS_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const THDM_LS_input_parameters& input
)
{
   THDM_LS_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void THDM_LS_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void THDM_LS_spectrum_impl<Solver_type>::fill_slha_io(THDM_LS_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   THDM_LS_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSTHDM_LSCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSTHDM_LSCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new THDM_LS_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSTHDM_LSGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSGetSettings"))
...
DLLEXPORT int FSTHDM_LSGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_LSGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_LSOpenHandle(
...
DLLEXPORT int FSTHDM_LSCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::THDM_LS_librarylink;
...
DLLEXPORT int FSTHDM_LSGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_LSToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_LSGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_LSCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_LSCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_LS_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_LSCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSTHDM_LSCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSTHDM_LSCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSTHDM_LSCalculateObservables", "error", e.what());
      put_error_output(link);
   }
