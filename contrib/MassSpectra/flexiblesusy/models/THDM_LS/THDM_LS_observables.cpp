...
const int THDM_LS_observables::NUMBER_OF_OBSERVABLES;

THDM_LS_observables::THDM_LS_observables()
...
Eigen::ArrayXd THDM_LS_observables::get() const
{
   Eigen::ArrayXd vec(THDM_LS_observables::NUMBER_OF_OBSERVABLES);
...
std::vector<std::string> THDM_LS_observables::get_names()
{
   std::vector<std::string> names(THDM_LS_observables::NUMBER_OF_OBSERVABLES);
...
void THDM_LS_observables::clear()
{
   eff_cp_higgs_photon_photon = Eigen::Array<std::complex<double>,2,1>::Zero();
   eff_cp_higgs_gluon_gluon = Eigen::Array<std::complex<double>,2,1>::Zero();
...
void THDM_LS_observables::set(const Eigen::ArrayXd& vec)
{
   assert(vec.rows() == THDM_LS_observables::NUMBER_OF_OBSERVABLES);
...
THDM_LS_observables calculate_observables(const THDM_LS_mass_eigenstates& model,
...
THDM_LS_observables calculate_observables(const THDM_LS_mass_eigenstates& model,
                                              ...
                                              const Physical_input& physical_input)
{
   THDM_LS_observables observables;

   THDM_LS_effective_couplings effective_couplings(model, qedqcd, physical_input);
