...
// File generated at Wed 31 Oct 2018 19:30:57

/**
 * @file THDM_lepton_a_muon.cpp
 *
 * This file was generated at Wed 31 Oct 2018 19:30:57 with FlexibleSUSY
 * 2.0.1 and SARAH 4.12.3 .
 */

#include "THDM_lepton_a_muon.hpp"
#include "THDM_lepton_mass_eigenstates.hpp"

#include "THDM_lepton_cxx_diagrams.hpp"
...
double get_MSUSY(const THDM_lepton_mass_eigenstates& model)
{
   return Min(model.get_MHm().tail<1>().minCoeff());

}

void run_to_MSUSY(THDM_lepton_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("THDM_lepton_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("THDM_lepton_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("THDM_lepton_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("THDM_lepton_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(THDM_lepton_mass_eigenstates& model)
{
   VERBOSE_MSG("THDM_lepton_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const THDM_lepton_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("THDM_lepton_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double THDM_lepton_a_muon::calculate_a_muon(const THDM_lepton_mass_eigenstates& model_)
{
   THDM_lepton_mass_eigenstates model(model_);

   VERBOSE_MSG("THDM_lepton_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDM_lepton_a_muon:" << e.what());
...
double THDM_lepton_a_muon::calculate_a_muon_uncertainty(const THDM_lepton_mass_eigenstates& model_)
{
   THDM_lepton_mass_eigenstates model(model_);

   VERBOSE_MSG("THDM_lepton_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDM_lepton_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
