...
// File generated at Wed 31 Oct 2018 19:31:01

#include "config.h"

#include "THDM_lepton_info.hpp"
#include "THDM_lepton_input_parameters.hpp"
#include "THDM_lepton_observables.hpp"
#include "THDM_lepton_physical.hpp"
#include "THDM_lepton_slha_io.hpp"
#include "THDM_lepton_model_slha.hpp"

#ifdef ENABLE_TWO_SCALE_SOLVER
#include "THDM_lepton_two_scale_model.hpp"
#include "THDM_lepton_two_scale_spectrum_generator.hpp"
#endif
...
namespace THDM_lepton_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSTHDM_leptonMessage", 1);
...
class THDM_lepton_spectrum {
public:
   virtual ~THDM_lepton_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(THDM_lepton_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const THDM_lepton_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_lepton_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class THDM_lepton_spectrum_impl : public THDM_lepton_spectrum
{
public:
   virtual ~THDM_lepton_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(THDM_lepton_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const THDM_lepton_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_lepton_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<THDM_lepton_slha<THDM_lepton<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   THDM_lepton_scales scales{};              ///< scale information
   THDM_lepton_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const THDM_lepton_input_parameters& input_) { input = input_; }
...
private:
   THDM_lepton_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<THDM_lepton_spectrum> spectrum{nullptr};  ///< spectrum information

   THDM_lepton_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, THDM_lepton_info::particle_names[i].c_str());
      }
   }
}
...
THDM_lepton_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   THDM_lepton_slha_io slha_io;
...
void put_spectrum(const THDM_lepton_slha<Model>& model, MLINK link)
{
   MLPutRule(link, THDM_lepton_info::model_name);
...
template <typename Solver_type>
void THDM_lepton_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void THDM_lepton_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const THDM_lepton_input_parameters& input
)
{
   THDM_lepton_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void THDM_lepton_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void THDM_lepton_spectrum_impl<Solver_type>::fill_slha_io(THDM_lepton_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   THDM_lepton_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSTHDM_leptonCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSTHDM_leptonCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new THDM_lepton_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSTHDM_leptonGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonGetSettings"))
...
DLLEXPORT int FSTHDM_leptonGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_leptonGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_leptonOpenHandle(
...
DLLEXPORT int FSTHDM_leptonCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;
...
DLLEXPORT int FSTHDM_leptonSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::THDM_lepton_librarylink;
...
DLLEXPORT int FSTHDM_leptonGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_leptonToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_leptonGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_leptonCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_leptonCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_lepton_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_leptonCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSTHDM_leptonCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSTHDM_leptonCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSTHDM_leptonCalculateObservables", "error", e.what());
      put_error_output(link);
   }
