...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSTHDM_IMessage", 1);
...
public:
   ...
   virtual void fill_slha_io(THDM_I_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const THDM_I_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_I_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<THDM_I_slha<THDM_I<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   THDM_I_scales scales{};              ///< scale information
   THDM_I_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const THDM_I_input_parameters& input_) { input = input_; }
...
private:
   THDM_I_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<THDM_I_spectrum> spectrum{nullptr};  ///< spectrum information

   THDM_I_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, THDM_I_info::particle_names[i].c_str());
      }
   }
}
...
THDM_I_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   THDM_I_slha_io slha_io;
...
void put_spectrum(const THDM_I_slha<Model>& model, MLINK link)
{
   MLPutRule(link, THDM_I_info::model_name);
...
template <typename Solver_type>
void THDM_I_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void THDM_I_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const THDM_I_input_parameters& input
)
{
...
template <typename Solver_type>
void THDM_I_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void THDM_I_spectrum_impl<Solver_type>::fill_slha_io(THDM_I_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   THDM_I_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSTHDM_ICalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSTHDM_ICalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new THDM_I_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSTHDM_IGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IGetSettings"))
...
DLLEXPORT int FSTHDM_IGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IOpenHandle(
...
DLLEXPORT int FSTHDM_ICloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::THDM_I_librarylink;
...
DLLEXPORT int FSTHDM_IGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_IGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_IGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_ICalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_ICalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_ICalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_I_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_ICalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSTHDM_ICalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSTHDM_ICalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSTHDM_ICalculateObservables", "error", e.what());
      put_error_output(link);
   }
