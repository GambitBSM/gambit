...
// File generated at Tue 7 Apr 2020 02:34:06

#include "THDM_I_two_scale_spectrum_generator.hpp"
#include "THDM_I_input_parameters.hpp"
#include "THDM_I_two_scale_convergence_tester.hpp"
...
double THDM_I_spectrum_generator<Two_scale>::get_pole_mass_scale() const
{
 ...
 */
void THDM_I_spectrum_generator<Two_scale>::run_except(const softsusy::QedQcd& qedqcd,
                                const THDM_I_input_parameters& input)
{
   ...
   THDM_I_ewsb_solver<Two_scale> ewsb_solver;
   model.set_ewsb_solver(
      std::make_shared<THDM_I_ewsb_solver<Two_scale> >(ewsb_solver));

   THDM_I_susy_scale_constraint<Two_scale> susy_scale_constraint(&model, qedqcd);
   THDM_I_low_scale_constraint<Two_scale>  low_scale_constraint(&model, qedqcd);

   susy_scale_constraint.initialize();
   low_scale_constraint .initialize();

   THDM_I_convergence_tester<Two_scale> convergence_tester(
   ...
   THDM_I_initial_guesser<Two_scale> initial_guesser(&model, qedqcd,
 ...
 */
void THDM_I_spectrum_generator<Two_scale>::write_running_couplings(
   const std::string& filename) const
{
   THDM_I_spectrum_generator_interface<Two_scale>::write_running_couplings(
      filename, get_low_scale(), get_susy_scale());
}

void THDM_I_spectrum_generator<Two_scale>::calculate_spectrum()
{
