...
/**
 * @file THDM_I_a_muon.cpp
 *
 * This file was generated at Wed 1 Apr 2020 20:45:51 with FlexibleSUSY
 * 2.0.1 and SARAH 4.13.0 .
 */

#include "THDM_I_a_muon.hpp"
#include "THDM_I_mass_eigenstates.hpp"

#include "THDM_I_cxx_diagrams.hpp"
...
double get_MSUSY(const THDM_I_mass_eigenstates& model)
{
   return Min(model.get_MHm().tail<1>().minCoeff());

}

void run_to_MSUSY(THDM_I_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("THDM_I_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("THDM_I_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("THDM_I_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("THDM_I_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(THDM_I_mass_eigenstates& model)
{
   VERBOSE_MSG("THDM_I_a_muon: calculating a_mu at Q = " << model.get_scale());
...
std::pair<double,double> vary_scale(const THDM_I_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("THDM_I_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double THDM_I_a_muon::calculate_a_muon(const THDM_I_mass_eigenstates& model_)
{
   THDM_I_mass_eigenstates model(model_);

   VERBOSE_MSG("THDM_I_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDM_I_a_muon:" << e.what());
...
double THDM_I_a_muon::calculate_a_muon_uncertainty(const THDM_I_mass_eigenstates& model_)
{
   THDM_I_mass_eigenstates model(model_);

   VERBOSE_MSG("THDM_I_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("THDM_I_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
