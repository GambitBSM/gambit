...
/**
 * @file THDM_I_mass_eigenstates.cpp
...
#include "THDM_I_ewsb_solver_interface.hpp"
...
#include "THDM_I_two_scale_ewsb_solver.hpp"
#endif
...
#define CLASSNAME THDM_I_mass_eigenstates
...
CLASSNAME::THDM_I_mass_eigenstates(const THDM_I_input_parameters& input_)
   : THDM_I_soft_parameters(input_)
#if defined(ENABLE_TWO_SCALE_SOLVER)
   , ewsb_solver(new THDM_I_ewsb_solver<Two_scale>())
#endif
{
}
...
const THDM_I_physical& CLASSNAME::get_physical() const
{
   return physical;
}

THDM_I_physical& CLASSNAME::get_physical()
{
   return physical;
}

void CLASSNAME::set_physical(const THDM_I_physical& physical_)
{
   physical = physical_;
}
...
void CLASSNAME::set_ewsb_solver(const std::shared_ptr<THDM_I_ewsb_solver_interface>& solver)
{
   ewsb_solver = solver;
}
...
{
   if (!ewsb_solver) {
      throw SetupError("THDM_I_mass_eigenstates::solve_ewsb_tree_level: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("THDM_I_mass_eigenstates::solve_ewsb_one_loop: "
                       "no EWSB solver set");
   }
...
{
   if (!ewsb_solver) {
      throw SetupError("THDM_I_mass_eigenstates::solve_ewsb: "
                       "no EWSB solver set");
   }

   VERBOSE_MSG("\t\tSolving THDM_I EWSB at " << ewsb_loop_order << "-loop order");
...
void CLASSNAME::print(std::ostream& ostr) const
{
   ostr << "========================================\n"
           "THDM_I\n"
           "========================================\n";
   THDM_I_soft_parameters::print(ostr);
   ostr << "----------------------------------------\n"
...
{
   if (PHYSICAL(Mhh).tail<2>().minCoeff() < 0.) problems.flag_pole_tachyon(THDM_I_info::hh);
   if (PHYSICAL(MAh).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(THDM_I_info::Ah);
   if (PHYSICAL(MHm).tail<1>().minCoeff() < 0.) problems.flag_pole_tachyon(THDM_I_info::Hm);
...
void CLASSNAME::run_to(double scale, double eps)
{
   if (eps < 0.0)
      eps = precision;
   THDM_I_soft_parameters::run_to(scale, eps);
}
...
void CLASSNAME::calculate_Mhh()
{
   ...
   problems.flag_bad_mass(THDM_I_info::hh, eigenvalue_error > precision *
      Abs(Mhh(0)));
#else
   ...
   if (Mhh.minCoeff() < 0.) {
      problems.flag_running_tachyon(THDM_I_info::hh);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   problems.flag_bad_mass(THDM_I_info::Ah, eigenvalue_error > precision *
      Abs(MAh(0)));
#else
   ...
   if (MAh.minCoeff() < 0.) {
      problems.flag_running_tachyon(THDM_I_info::Ah);
   }
...
void CLASSNAME::calculate_MHm()
{
   ...
   problems.flag_bad_mass(THDM_I_info::Hm, eigenvalue_error > precision *
      Abs(MHm(0)));
#else
   ...
   if (MHm.minCoeff() < 0.) {
      problems.flag_running_tachyon(THDM_I_info::Hm);
   }
...
void CLASSNAME::calculate_MFd()
{
   ...
   problems.flag_bad_mass(THDM_I_info::Fd, eigenvalue_error > precision *
      Abs(MFd(0)));
#else
...
void CLASSNAME::calculate_MFu()
{
   ...
   problems.flag_bad_mass(THDM_I_info::Fu, eigenvalue_error > precision *
      Abs(MFu(0)));
#else
...
void CLASSNAME::calculate_MFe()
{
   ...
   problems.flag_bad_mass(THDM_I_info::Fe, eigenvalue_error > precision *
      Abs(MFe(0)));
#else
...
void CLASSNAME::calculate_MVWm()
{
   ...
   if (MVWm < 0.) {
      problems.flag_running_tachyon(THDM_I_info::VWm);
   }
...
{
   const std::complex<double> result = std::complex<double>(0.,
      -0.7071067811865475)*KroneckerDelta(1,gO2)*SUM(j1,0,2,Conj(Ye(j1,gI1))*Ue(
      gI2,j1));
...
{
   const std::complex<double> result = IF(gO2 < 3,std::complex<double>(0.,
      0.7071067811865475)*Ye(gO2,gI1)*ZA(gI2,1),0);
...
{
   const std::complex<double> result = std::complex<double>(0.,
      -0.7071067811865475)*SUM(j1,0,2,Conj(Ye(j1,gO1))*Ue(gI1,j1))*ZA(gI2,1);
...
std::complex<double> CLASSNAME::CpbarFeFeconjHmPL(int gO2, int gI2, int gI1) const
{
   const std::complex<double> result = -(SUM(j2,0,2,Conj(Ve(gI2,j2))*SUM(j1,0,2
      ,Conj(Ue(gO2,j1))*Ye(j1,j2)))*ZP(gI1,1));
...
{
   const std::complex<double> result = std::complex<double>(0.,
      0.7071067811865475)*SUM(j1,0,2,Conj(Ue(gO2,j1))*Ye(j1,gI1))*ZA(gI2,1);
...
{
   if (!force_output && problems.is_running_tachyon(THDM_I_info::VZ))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(THDM_I_info::VZ);
...
{
   if (!force_output && problems.is_running_tachyon(THDM_I_info::hh))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZH;
            ...
            problems.flag_bad_mass(THDM_I_info::hh,
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(THDM_I_info::hh);
   else
      problems.unflag_no_pole_mass_convergence(THDM_I_info::hh);
}

void CLASSNAME::calculate_MAh_pole()
{
   if (!force_output && problems.is_running_tachyon(THDM_I_info::Ah))
      return;
   ...
   do {
      ...
      for (int es = 0; es < 2; ++es) {
         ...
         Eigen::Matrix<double,2,2> mix_ZA;
            ...
            problems.flag_bad_mass(THDM_I_info::Ah,
               eigenvalue_error > precision * Abs(eigen_values(0)));
         #else
            fs_diagonalize_hermitian(M_loop, eigen_values,
               mix_ZA);
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(THDM_I_info::Ah);
   else
      problems.unflag_no_pole_mass_convergence(THDM_I_info::Ah);
}

void CLASSNAME::calculate_MHm_pole()
{
   if (!force_output && problems.is_running_tachyon(THDM_I_info::Hm))
      return;
   ...
   for (int es = 0; es < 2; ++es) {
      ...
      Eigen::Matrix<double,2,2> mix_ZP;
         ...
         fs_diagonalize_hermitian(M_loop, eigen_values, mix_ZP,
            eigenvalue_error);
         problems.flag_bad_mass(THDM_I_info::Hm, eigenvalue_error >
...
void CLASSNAME::calculate_MFd_pole()
{
   ...
   for (int es = 0; es < 3; ++es) {
      ...
      problems.flag_bad_mass(THDM_I_info::Fd, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFu_pole()
{
   ...
   for (int es = 0; es < 3; ++es) {
      ...
      problems.flag_bad_mass(THDM_I_info::Fu, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
void CLASSNAME::calculate_MFe_pole()
{
   ...
   for (int es = 0; es < 3; ++es) {
      ...
      problems.flag_bad_mass(THDM_I_info::Fe, eigenvalue_error >
         precision * Abs(eigen_values(0)));
   #else
...
{
   if (!force_output && problems.is_running_tachyon(THDM_I_info::VWm))
      return;
   ...
   if (mass_sqr < 0.)
      problems.flag_pole_tachyon(THDM_I_info::VWm);
...
std::ostream& operator<<(std::ostream& ostr, const THDM_I_mass_eigenstates& model)
{
   model.print(ostr);
   return ostr;
}
