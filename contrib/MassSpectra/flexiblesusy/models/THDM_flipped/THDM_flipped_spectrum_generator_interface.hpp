...
#ifndef THDM_flipped_SPECTRUM_GENERATOR_INTERFACE_H
#define THDM_flipped_SPECTRUM_GENERATOR_INTERFACE_H

#include "THDM_flipped_mass_eigenstates.hpp"
#include "THDM_flipped_model.hpp"
...
template <class T>
class THDM_flipped_spectrum_generator_interface {
public:
   virtual ~THDM_flipped_spectrum_generator_interface() = default;

   std::tuple<THDM_flipped<T>> get_models() const
   { return std::make_tuple(model); }
   std::tuple<THDM_flipped_slha<THDM_flipped<T>>> get_models_slha() const
   { return std::make_tuple(THDM_flipped_slha<THDM_flipped<T> >(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.)); }

   THDM_flipped<T> get_model() const
   { return model; }
   THDM_flipped_slha<THDM_flipped<T>> get_model_slha() const
   { return THDM_flipped_slha<THDM_flipped<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const THDM_flipped_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "THDM_flipped_spectrum.dat") const;

protected:
   THDM_flipped<T> model;
   ...
   void translate_exception_to_problem(THDM_flipped<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const THDM_flipped_input_parameters&) = 0;
};
 ...
 */
template <class T>
void THDM_flipped_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void THDM_flipped_spectrum_generator_interface<T>::run(
   const softsusy::QedQcd& qedqcd_, const THDM_flipped_input_parameters& input)
{
 ...
 */
template <class T>
void THDM_flipped_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   THDM_flipped_mass_eigenstates tmp_model(model);
   try {
   ...
   THDM_flipped_parameter_getter parameter_getter;
   Coupling_monitor<THDM_flipped_mass_eigenstates, THDM_flipped_parameter_getter>
 ...
 */
template <class T>
void THDM_flipped_spectrum_generator_interface<T>::write_spectrum(
   const std::string& filename) const
{
   THDM_flipped_spectrum_plotter plotter(model);
 ...
 */
template <class T>
void THDM_flipped_spectrum_generator_interface<T>::translate_exception_to_problem(THDM_flipped<T>& model)
{
   try {
      throw;
