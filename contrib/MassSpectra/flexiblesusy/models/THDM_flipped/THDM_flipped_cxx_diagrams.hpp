...
/**
 * @file THDM_flipped_cxx_diagrams.hpp
 *
 * This file was generated at Wed 31 Oct 2018 21:01:58 with FlexibleSUSY
 * 2.0.1 and SARAH 4.12.3 .
 */

#ifndef THDM_flipped_CXXDIAGRAMS_H
#define THDM_flipped_CXXDIAGRAMS_H
...
*/
struct EvaluationContext {
   THDM_flipped_mass_eigenstates& model; ///< The model object.
...
Vertex<VP, bar<Fe>::type, Fe>::evaluate(const indices_type& indices, const EvaluationContext& context)
{
   ...
   const std::complex<double> left = 0.5*KroneckerDelta(gt2,gt3)*(g1*Cos(ThetaW) + g2*Sin(ThetaW));

   const std::complex<double> right = g1*Cos(ThetaW)*KroneckerDelta(gt2,gt3);
...
namespace impl {
   static LeftAndRightComponentedVertex unit_charge(const EvaluationContext& context)
   {
      ...
      const std::complex<double> left = -(g1*Cos(ThetaW)*KroneckerDelta(gt2,gt3));

      const std::complex<double> right = -0.5*KroneckerDelta(gt2,gt3)*(g1*Cos(ThetaW) + g2*Sin(ThetaW));
