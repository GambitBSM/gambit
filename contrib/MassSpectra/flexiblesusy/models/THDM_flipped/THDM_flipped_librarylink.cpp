...
// File generated at Wed 31 Oct 2018 21:01:58
...
#include "THDM_flipped_slha_io.hpp"
#include "THDM_flipped_model_slha.hpp"
...
namespace THDM_flipped_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSTHDM_flippedMessage", 1);
...
class THDM_flipped_spectrum {
public:
   virtual ~THDM_flipped_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(THDM_flipped_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const THDM_flipped_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_flipped_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class THDM_flipped_spectrum_impl : public THDM_flipped_spectrum
{
public:
   virtual ~THDM_flipped_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(THDM_flipped_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const THDM_flipped_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const THDM_flipped_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<THDM_flipped_slha<THDM_flipped<Solver_type>>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   THDM_flipped_scales scales{};              ///< scale information
   THDM_flipped_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const THDM_flipped_input_parameters& input_) { input = input_; }
...
private:
   THDM_flipped_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<THDM_flipped_spectrum> spectrum{nullptr};  ///< spectrum information

   THDM_flipped_slha_io get_slha_io() const;
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, THDM_flipped_info::particle_names[i].c_str());
      }
   }
}
...
THDM_flipped_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   THDM_flipped_slha_io slha_io;
...
void put_spectrum(const THDM_flipped_slha<Model>& model, MLINK link)
{
   MLPutRule(link, THDM_flipped_info::model_name);
...
template <typename Solver_type>
void THDM_flipped_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void THDM_flipped_spectrum_impl<Solver_type>::calculate_spectrum(
   const Spectrum_generator_settings& settings,
   const SLHA_io::Modsel& modsel,
   const softsusy::QedQcd& qedqcd,
   const THDM_flipped_input_parameters& input
)
{
   THDM_flipped_spectrum_generator<Solver_type> spectrum_generator;
...
template <typename Solver_type>
void THDM_flipped_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void THDM_flipped_spectrum_impl<Solver_type>::fill_slha_io(THDM_flipped_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   THDM_flipped_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSTHDM_flippedCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSTHDM_flippedCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_TWO_SCALE_SOLVER
   case 1:
      spectrum.reset(new THDM_flipped_spectrum_impl<Two_scale>());
...
DLLEXPORT int FSTHDM_flippedGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedGetSettings"))
...
DLLEXPORT int FSTHDM_flippedGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_flippedGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_flippedOpenHandle(
...
DLLEXPORT int FSTHDM_flippedCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;
...
DLLEXPORT int FSTHDM_flippedSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::THDM_flipped_librarylink;
...
DLLEXPORT int FSTHDM_flippedGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_flippedToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_flippedGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_flippedCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSTHDM_flippedCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::THDM_flipped_librarylink;

   if (!check_number_of_args(link, 1, "FSTHDM_flippedCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSTHDM_flippedCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSTHDM_flippedCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSTHDM_flippedCalculateObservables", "error", e.what());
      put_error_output(link);
   }
