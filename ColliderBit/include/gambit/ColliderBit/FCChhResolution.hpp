...
///
///  \file
///  Resolution functions for FCC-hh, adapted from https://github.com/delphes/delphes/blob/master/cards/FCC and https://cds.cern.ch/record/2717698/files/CERN-FCC-PHYS-2020-0003.pdf
///
///  *********************************************
///
...
namespace Gambit
{
  ...
  namespace ColliderBit
  {
    ...
    namespace FCChh
    {
      inline double electronMomentumResolution(double eta, double energy) {
      	/// Relative resolution in tracker
      	const double tracker_res = 1.5 * ( (abs(eta) >= 0.0000 && abs(eta) < 0.1000) * (energy >= 0.0000 && energy < 1.0000) * (0.00315864) + 
		...
		/// Relative uncertainty in calorimeter
		const double Scoeff = (abs(eta) < 1.7) * 0.1 + (abs(eta) >= 1.7 && abs(eta) < 4.0) * 0.1 + (abs(eta) >= 4.0 && abs(eta) < 6.0) * 0.3;
		const double Ccoeff = (abs(eta) < 1.7) * 0.007 + (abs(eta) >= 1.7 && abs(eta) < 4.0) * 0.007 + (abs(eta) >= 4.0 && abs(eta) < 6.0) * 0.035;
		const double cal_res = sqrt(Scoeff*Scoeff/energy + Ccoeff*Ccoeff);

		return std::min(cal_res,tracker_res);
      }

      inline double muonMomentumResolution(double eta, double energy) {
      	/// Relative uncertainty in tracker
      	const double pt = energy/cosh(eta);
      	const double tracker_res = ( abs(eta) >= 0 && abs(eta) <= 1 ) *
		(sqrt(1e-5/pow(sin(2*atan(exp(-abs(eta)))),2) + (
		3*9.06262e-8 *pt*pt* pow(cosh(
		eta),2) *(2.82074e-7/pow(sin(2*atan(exp(-abs(eta)))),2) + (
		504.525 *(1/400000000 + (0.117945* 1/pow(cosh(eta),2))/(
		pt*pt *pow(sin(2*atan(exp(-abs(eta)))),2))))/
		pow(sin(2*atan(exp(-abs(eta)))),2)) *pow(sin(2*atan(exp(-abs(eta)))),2))/(
		0.00516429/pow(sin(2*atan(exp(-abs(eta)))),2) + (
		96868.8 *(1/400000000 + 5*(0.117945 * 1/pow(cosh(eta),2))/(
		pt*pt *pow(sin(2*atan(exp(-abs(eta)))),2))))/
		pow(sin(2*atan(exp(-abs(eta)))),2)))
		) +
		( abs(eta) > 1 && abs(eta) < 1.30 ) *
    	(sqrt(1e-5/pow(tan(2*atan(exp(-abs(eta)))),2) + (
        3*9.06262e-8 *pt*pt* pow(cosh(
        eta),2) *(2.82074e-7/pow(sin(2*atan(exp(-abs(eta)))),2) + (
        504.525 *(1/400000000 + (0.117945* 1/pow(cosh(eta),2))/(
        pt*pt *pow(sin(2*atan(exp(-abs(eta)))),2))))/
		...
		/// No calorimeter in this case
		return tracker_res;
      }

      inline double jetMomentumResolution(double eta, double energy) {
      	double res;
      	if(abs(eta) < 1.5) {
      	  /// Use particle-flow resolution from Fig. 2 in https://cds.cern.ch/record/2717698/files/CERN-FCC-PHYS-2020-0003.pdf (roughly)
      	  res = (energy < 15) * 0.06 + (energy >= 15 && energy < 20) * 0.053 + (energy >= 20 && energy < 30) * 0.045 +
      	  (energy >= 30 && energy < 50) * 0.038 + (energy >= 50 && energy < 75) * 0.03 +
      	  (energy >= 75 && energy < 125) * 0.023 + (energy >= 125 && energy < 200) * 0.017 +
      	  (energy >= 200 && energy < 300) * 0.014 + (energy >= 300 && energy < 500) * 0.012 +
      	  (energy >= 500 && energy < 1000) * 0.01 + (energy >= 1000 && energy < 1500) * 0.012 +
      	  (energy >= 1500 && energy < 2500) * 0.015 + (energy >= 2500 && energy < 4000) * 0.018 +
      	  (energy >= 4000 && energy < 10000) * 0.025 + (energy >= 10000) * 0.03;
      	}
      	else{
      	  /// Relative uncertainty in hadronic calorimeter since no tracker corrections are avaliable for these eta-values, probably conservative
		  const double Scoeff = (abs(eta) < 1.7) * 0.5 + (abs(eta) >= 1.7 && abs(eta) < 4.0) * 0.6 + (abs(eta) >= 4.0 && abs(eta) < 6.0) * 1.0;
		  const double Ccoeff = (abs(eta) < 1.7) * 0.03 + (abs(eta) >= 1.7 && abs(eta) < 4.0) * 0.03 + (abs(eta) >= 4.0 && abs(eta) < 6.0) * 0.1;
		  res = sqrt(Scoeff*Scoeff/energy + Ccoeff*Ccoeff);
      	}
      	return res;
      }
