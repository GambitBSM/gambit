...
#define MODULE ColliderBit
  ...
  // TODO: Perhaps comment this out since I have a version that should be generated by GUM
  
  #define CAPABILITY JetMatchedEvent
  START_CAPABILITY
  
    // Trivial function that does nothing to the event for when no jetmatching is requested
    #define FUNCTION nojetmatching
    START_FUNCTION(HEPUtils::Event)
    DEPENDENCY(HardScatteringEvent, HEPUtils::Event)
    DEPENDENCY(HardScatteringSim, Py8Collider_defaultversion)
    #undef FUNCTION
  
    // A first simple attempt at creating the right inputs/outputs for later development
    // TODO: This is an old version. The new version is generated with preprocessor directives
    //#define FUNCTION jetmatching_dummy
    //START_FUNCTION(HEPUtils::Event) 
    //DEPENDENCY(HardScatteringEvent, HEPUtils::Event)
    //DEPENDENCY(HardScatteringSim, Py8Collider_defaultversion)
    //#undef FUNCTION
