...
namespace Gambit
{
  namespace ColliderBit
  {
    ...
    private:
      ...
      static void trimJets(const vector<FJNS::PseudoJet> &pjs_in, vector<Jet> &largeJetsOut, const double frac = 0.00, const double minPt = 0., const double maxEta = 6.,
                           const double minPt_twosubJets = 0.)
      {
        largeJetsOut.clear();
        for (const FJNS::PseudoJet &pj : pjs_in)
        {
          if (pj.pt() < minPt || abs(pj.eta()) > maxEta || (pj.pt() < minPt_twosubJets && pj.constituents().size() == 1)) continue;
          ...
          if (running_total.pT() < minPt || running_total.abseta() > maxEta || (running_total.pT() < minPt_twosubJets && preserved_subjets.size() == 1)) continue;
    ...
    public:
      ...
      void run(const HEPUtils::Event *event)
      {
        ...
        auto vrbtag = generateBTagsMap(VR_jets, 0.70119, 0.09639, 0.00390);
        for (const HEPUtils::Jet *jet : VR_jets)
        {
          bool isbtag = vrbtag.at(jet);
          if (isbtag && jet->abseta() < 2.5 && jet->pT() > 25.) { bTagged_Jets.push_back(jet); }
        }
        ...
        for (size_t i = 0; i < largeR_Jets.size(); ++i)
        {
          ...
          HEPUtils::Jet *hepUtilsJet = owned_trimmed_jets.back().get();
          if (toptag)
          {
        ...
        if (higgsJets.size() >= 1)
        {
          ...
          {
            if (jet->mom().deltaR_eta(higgsJets.at(0)->mom()) < 1.0) { nbHiggs += 1; }
          }
        }
        ...
        if (topJets.size() >= 1)
        {
          for (const HEPUtils::Jet *jet : bTagged_Jets)
          {
            if (jet->mom().deltaR_eta(topJets.at(0)->mom()) < 1.0) { nbtop += 1; }
          }
        }
        bool srht = (tid == 0 && hid == 1) || (tid == 1 && hid == 0);
        bool srnb = srht && (nbtop >= 1) && (nbHiggs >= 2);
        double mHt = (srht && !higgsJets.empty() && !topJets.empty())
                       ? (higgsJets.at(0)->mom() + topJets.at(0)->mom()).m()
                       : 0.;
