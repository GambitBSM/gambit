   ...
   private:
    ...
    class Analysis_ATLAS_13TeV_2LEP0JET_EW_139invfb : public Analysis
    {
	 ...
	 return m_MVAs[name];
      ...
      vector<const Particle*> filterLeptons(vector<const Particle*> &cands,
                                float ptCut, float etaCut) {
      
	vector<const Particle*> reducedList;
	for (const auto &cand : cands) {
	  if ((cand->pT() >= ptCut) && (cand->abseta() < etaCut))
	...
	return reducedList;
      ...
      // Assuming Particle and Particles are correct type names.
      vector<const Jet*> filterJets(vector<const Jet*> &cands,
                                float ptCut, float etaCut) {
	...
	bool operator()(const Particle* p1, const Particle* p2) const {
      ...
      int countObjects(vector<const Particle*> &cands, float ptCut,
                                float etaCut) {
	int count = 0;
	for (const auto &cand : cands) {
	  if (cand->pT() >= ptCut && cand->abseta() < etaCut)
	...
	return count;
      ...
      int countObjects(vector<const Jet*> &cands, float ptCut,
                                float etaCut) {
	int count = 0;
	for (const auto &cand : cands) {
	  if (cand->pT() >= ptCut && cand->abseta() < etaCut)
	...
	return count;
      ...
      float calcMT2(const Particle &o1, const Particle &o2,
                             const P4 &met) {
	    return calcAMT2(o1, o2, met, 0, 0);
      }

      float calcAMT2(const Particle &o1,
                              const Particle &o2,
                              const P4 &met, float m1, float m2) {
	    asymm_mt2_lester_bisect::disableCopyrightMessage();
	    return asymm_mt2_lester_bisect::get_mT2(o1.mom().m(), o1.mom().px(), o1.mom().py(), o2.mom().m(),
                                          o2.mom().px(), o2.mom().py(), met.px(), met.py(),
                                          m1, m2);
      }

      float calcMTauTau(const Particle &o1,
                                 const Particle &o2,
                                 const P4 &met) {
	...
	return reducedList;
      ...
      vector<const Jet*> JetLeptonOverlapRemoval(vector<const Jet*> &cands, vector< const Particle*> &others,
			  std::function<float(const Jet* , const Particle* )>
	                  radiusFunc) {
	...
	return reducedList;
      ...
      vector<const Particle*> LeptonJetOverlapRemoval(vector<const Particle*> &cands, vector< const Jet*> &others,
			  std::function<float(const Particle* , const Jet* )>
	                  radiusFunc) {
	...
	return reducedList;
      ...
      vector<const Particle*> LeptonLeptonOverlapRemoval(vector<const Particle*> &cands,
                                        vector<const Particle*> &others,
                                              float deltaR) {
	return LeptonLeptonOverlapRemoval(
			      cands, others,
			      [deltaR](const Particle*, const Particle*) {
				return deltaR;
			      }
			     );
      }
      ...
      vector<const Jet*> JetLeptonOverlapRemoval(vector<const Jet*> &cands,
                                        vector<const Particle*> &others,
                                              float deltaR) {
	return JetLeptonOverlapRemoval(
			      cands, others,
			      [deltaR](const Jet*, const Particle*) {
				return deltaR;
			      }
			     );
      }
      ...
      vector<const Particle*> LeptonJetOverlapRemoval(vector<const Particle*> &cands,
                                        vector<const Jet*> &others,
                                              float deltaR) {
	return LeptonJetOverlapRemoval(
			      cands, others,
			      [deltaR](const Particle*, const Jet*) {
				return deltaR;
			      }
			     );
      }
	...
	if (applyOverlapRemoval) { // This is a default overlap removal
	  ...
	  auto radiusCalcLepton = [](const Particle* lepton,
                               const Jet* jet) {
				    return std::min(0.4, 0.04 + 10 / lepton->pT());
				  };

	  auto muJetSpecial = [](const Jet* jet,
				 const Particle* muon) {
				if (muon->pT() / jet->pT() > 0.5)
				  return 0.2;
				else
				  return 0.;
			      };
	...
	if (type == "electrons") {
         ...
         for (const Particle* electron : event->electrons())
          if (electron->pT() > ptcut && electron->abseta() < etacut)
            cands.push_back(electron);
	 //return cands;
        }
	...
	if (type == "muons") {
         ...
         for (const Particle* muon : event->muons())
          if (muon->pT() > ptcut && muon->abseta() < etacut)
	...
	if (type == "photons") {
         ...
         for (const Particle* photon : event->photons())
          if (photon->pT() > ptcut && photon->abseta() < etacut)
            cands.push_back(photon);
	 //return cands;
        }
	...
	if (type == "taus") {
         ...
         for (const Particle* tau : event->taus())
          if (tau->pT() > ptcut && tau->abseta() < etacut)
            cands.push_back(tau);
	 //return cands;
        }
      ...
      vector<const Jet*> getJets(const HEPUtils::Event* event, double ptcut, double etacut) {
         ...
         for (const Jet* jet : event->jets("antikt_R04"))
          if (jet->pT() > ptcut && jet->abseta() < etacut)
