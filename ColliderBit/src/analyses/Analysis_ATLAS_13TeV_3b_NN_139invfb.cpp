  ...
  namespace ColliderBit {
    ...
    public:
      ...
      static void trimJets(const vector<FJNS::PseudoJet> &pjs_in, vector<Jet> &js_out,
                           const double frac = 0.1, const double minPt = 0., const double maxEta = 6.){
        js_out.clear();
        for (const FJNS::PseudoJet & pj : pjs_in){
          if (pj.pt() < minPt || abs(pj.eta()) > maxEta) continue;
          ...
          if (running_total.pT() < minPt || running_total.abseta() > maxEta) continue;
      ...
      Analysis_ATLAS_13TeV_3b_NN_139invfb() {
        ...
        _nn = std::make_unique<onnx_rt_wrapper>(GAMBIT_DIR"/ColliderBit/data/analyses_ML/Analysis_ATLAS_13TeV_3b_NN_139invfb.onnx");
        ...
        _cutflows.addCutflow("SR_Gbb_B", {"Nsiglep=0","dPhi4jmin>0.4","mbjets_Tmin>=130","ETMiss>=550","pTjet>=65","Meff>=2600"}); //TODO: bjets????
        _cutflows.addCutflow("SR_Gbb_M", {"Nsiglep=0","dPhi4jmin>0.4","mbjets_Tmin>=130","ETMiss>=550","Meff>=2000"});
        _cutflows.addCutflow("SR_Gbb_C", {"Nsiglep=0","dPhi4jmin>0.4","mbjets_Tmin>=130","Meff>=1600","ETMiss>=550"}); 
        ...
        //TODO: the ordering of the bjet cut confuses me.
      ...
      void run(const HEPUtils::Event* event) {
        ...
        vector<const HEPUtils::Particle*> sigElectrons = electrons;
        vector<const HEPUtils::Particle*> sigMuons = muons;
        // Apply cuts to get signal electrons
        // Apply electron isolation efficiency from "Tight" criteria in 1908.00005
        ATLAS::applyElectronIsolationEfficiency2020(sigElectrons, "Tight");
        ...
        ATLAS::applyMuonIsolationEfficiency2020(sigMuons, "Tight");
        ...
        size_t nBaseLeptons = electrons.size() + muons.size();
        ...
        ///////////////////////////////////////////////////////////////////////
        // NN analysis
        // n.b. float is an onnx necessity
        ...
        if (nBaseLeptons == 0 && dPhi4jmin > 0.4 ){
          ...
          if (dPhi4jmin >= 0.4 && nn_outputs["Gbb_2300_1000"][1] > 0.9994){
        ...
        if ((nBaseLeptons == 0 && dPhi4jmin >= 0.4) || nSigLeptons >= 1){
          _cutflows["SR_Gtt_2100_1"].fill(1, {(nBaseLeptons == 0 && dPhi4jmin >= 0.4) || nSigLeptons == 1, nn_outputs["Gtt_2100_1"][0] > 0.9997});
          _cutflows["SR_Gtt_1800_1"].fill(1, {(nBaseLeptons == 0 && dPhi4jmin >= 0.4) || nSigLeptons == 1, nn_outputs["Gtt_1800_1"][0] > 0.9997});
          _cutflows["SR_Gtt_2300_1200"].fill(1, {(nBaseLeptons == 0 && dPhi4jmin >= 0.4) || nSigLeptons == 1, nn_outputs["Gtt_2300_1200"][0] > 0.9993});
          _cutflows["SR_Gtt_1900_1400"].fill(1, {(nBaseLeptons == 0 && dPhi4jmin >= 0.4) || nSigLeptons == 1, nn_outputs["Gtt_1900_1400"][0] > 0.9987});
        }
        if (nBaseLeptons == 0){
          if (nBaseLeptons == 0){
            _cutflows["SR_Gbb_2100_1600"].fill(1);
            _cutflows["SR_Gbb_2000_1800"].fill(1);
            _cutflows["SR_Gbb_2800_1400"].fill(1);
            _cutflows["SR_Gbb_2300_1000"].fill(1);
            if (dPhi4jmin >= 0.4){
              _cutflows["SR_Gbb_2100_1600"].fill(2, {dPhi4jmin >= 0.4, nn_outputs["Gbb_2100_1600"][1] > 0.9993});
              _cutflows["SR_Gbb_2000_1800"].fill(2, {dPhi4jmin >= 0.4, nn_outputs["Gbb_2000_1800"][1] > 0.997});
            }
            if (dPhi4jmin >= 0.6){
              _cutflows["SR_Gbb_2800_1400"].fill(2, {dPhi4jmin >= 0.6, nn_outputs["Gbb_2800_1400"][1] > 0.999});
              _cutflows["SR_Gbb_2300_1000"].fill(2, {dPhi4jmin >= 0.6, nn_outputs["Gbb_2300_1000"][1] > 0.9994});
            }
          }
        }
        #endif //CHECK_CUTFLOWS
        ...
        if (nBaseLeptons == 0){
          ...
          #ifdef CHECK_CUTFLOWS
          _cutflows["SR_Gbb_B"].fill(1, {true, dPhi4jmin>=0.4, m_transverse_B_min>=130, met>=550, smallJets[0].pT() >= 65, meff >= 2600});
          _cutflows["SR_Gbb_M"].fill(1, {true, dPhi4jmin>=0.4, m_transverse_B_min>=130, met>=550, meff >= 2000});
          _cutflows["SR_Gbb_C"].fill(1, {true, dPhi4jmin>=0.4, m_transverse_B_min>=130, meff >= 1600, met>=550});

          _cutflows["SR_Gtb_B"].fill(2, {true, dPhi4jmin>=0.4, m_transverse_B_min>=130, meff>=2500, met >=550, totJetMass >= 200});
          _cutflows["SR_Gtb_M"].fill(2, {true, dPhi4jmin>=0.4, m_transverse_B_min>=130, nJets>=6, nbJets>=4, meff>=2000, met >=550, totJetMass >= 200});
          _cutflows["SR_Gtb_C"].fill(2, {true, dPhi4jmin>=0.4, m_transverse_B_min>=130, nJets>=7, nbJets>=4, meff>=1300, met >=500, totJetMass >= 50});

          _cutflows["SR_Gtt_0l_B"].fill(1, {true, dPhi4jmin>=0.4, nJets>=5, met>=600, meff>=2900, m_transverse_B_min>=120, totJetMass >= 300});
          _cutflows["SR_Gtt_0l_M1"].fill(1, {true, dPhi4jmin>=0.4, nJets>=9, met>=600, meff>=1700, m_transverse_B_min>=120, totJetMass >= 300});
          _cutflows["SR_Gtt_0l_M2"].fill(1, {true, dPhi4jmin>=0.4, nJets>=10, met>=600, meff>=1100, m_transverse_B_min>=120, totJetMass >= 200});
          _cutflows["SR_Gtt_0l_C"].fill(1, {true, dPhi4jmin>=0.4, nJets>=10, nbJets>=4, met>=400, meff>=800, m_transverse_B_min>=180, totJetMass >= 100});
          #endif //CHECK_CUTFLOWS
        }
        // 1 lepton channel
          ...
          #ifdef CHECK_CUTFLOWS
            _cutflows["SR_Gtt_1l_B"].fill(1,{true, nJets >= 4 && nbJets>=3, met >= 600, meff >= 2300, m_transverse >= 150, m_transverse_B_min >= 120, totJetMass >= 200});
            _cutflows["SR_Gtt_1l_M1"].fill(1,{true, nJets >= 5 && nbJets>=3, met >= 600, meff >= 2000, m_transverse >= 200, m_transverse_B_min >= 120, totJetMass >= 200});
            _cutflows["SR_Gtt_1l_M2"].fill(1,{true, nJets >= 8 && nbJets>=3, met >= 500, meff >= 1100, m_transverse >= 200, m_transverse_B_min >= 120, totJetMass >= 200});
            _cutflows["SR_Gtt_1l_C"].fill(1,{true, nJets >= 9 && nbJets>=3, met >= 300, meff >= 800, m_transverse >= 150, m_transverse_B_min >= 120});
          #endif  
        }
      ...
      {
        const Analysis_ATLAS_13TeV_3b_NN_139invfb* specificOther
          = dynamic_cast<const Analysis_ATLAS_13TeV_3b_NN_139invfb*>(other);
    ...
    DEFINE_ANALYSIS_FACTORY(ATLAS_13TeV_3b_NN_139invfb)
