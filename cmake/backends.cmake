...
if(NOT ditched_${name}_${ver})
  ExternalProject_Add(${name}_${ver}
    ...
    PATCH_COMMAND patch -p1 < ${patch}
    CONFIGURE_COMMAND ""
    BUILD_COMMAND ${MAKE_SERIAL} CC=${CMAKE_C_COMPILER} ARFLAGS=rcsU CFLAGS=${AlterBBN_C_FLAGS} CFLAGS_MP=${OpenMP_C_FLAGS}
...
if(NOT ditched_${name}_${ver})
  ...
  if(modules_missing_${name}_${ver})
    ...
    ExternalProject_Add(${name}_${ver}
      ...
      COMMAND sed ${dashi} -e "s#rm -f libclass.a#rm -rf libclass.a lib#g" Makefile
      COMMAND sed ${dashi} -e "s#\"[.]\"#\"${dir}\"#g" include/common.h
      #BUILD_COMMAND ${MAKE_PARALLEL} CC=${CMAKE_C_COMPILER} OMPFLAG=${OpenMP_C_FLAGS} OPTFLAG= LDSHARED=${LDSHARED} LDFLAGS=${BACKEND_C_FLAGS} CXXFLAGS=${BACKEND_CXX_FLAGS} CCFLAGS=${BACKEND_C_FLAGS} CCFLAG=${BACKEND_C_FLAGS} LDFLAG=${BACKEND_C_FLAGS} PYTHON=${PYTHON_EXECUTABLE} all
      #BUILD_COMMAND ${MAKE_PARALLEL} CC=${CMAKE_C_COMPILER} OMPFLAG=${CLASSY_OpenMP_C_FLAGS} OPTFLAG= CCFLAG=${BACKEND_C_FLAGS} LDFLAG=${CMAKE_SHARED_LINKER_FLAGS} PYTHON=${PYTHON_EXECUTABLE} all
      BUILD_COMMAND ${MAKE_PARALLEL} CC=${CMAKE_C_COMPILER} OMPFLAG=${CLASSY_OpenMP_C_FLAGS} OPTFLAG= CCFLAG=${BACKEND_C_FLAGS} LDFLAG=${CMAKE_SHARED_LINKER_FLAGS} PYTHON=${PYTHON_EXECUTABLE} LDSHARED=${LDSHARED} LDFLAGS=${CMAKE_SHARED_LINKER_FLAGS} CXXFLAGS=${BACKEND_CXX_FLAGS} CCFLAGS=${BACKEND_C_FLAGS}   all
      COMMAND ${CMAKE_COMMAND} -E make_directory lib
      COMMAND find python/ -name "classy*.so" | xargs -I {} cp "{}" lib/
      COMMAND ${CMAKE_COMMAND} -E echo "#This is a trampoline script to import the cythonized python module under a different name" > lib/${lib}_${sfver}.py
      COMMAND ${CMAKE_COMMAND} -E echo "from ${lib} import *" >> lib/${lib}_${sfver}.py
