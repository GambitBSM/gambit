...
# An explicit -DWITH_HEPMC=OFF is honoured even when ColliderBit is in use, for
# environments where the HepMC download is impossible; HepMC-dependent functions
# in ColliderBit, CBS, and HepMC-dependent backends are then disabled.
if(";${GAMBIT_BITS};" MATCHES ";ColliderBit;")
  if(DEFINED WITH_HEPMC AND NOT WITH_HEPMC)
    message("${BoldRed}   WITH_HEPMC=OFF requested: building ColliderBit without HepMC.${ColourReset}")
    set(WITH_HEPMC OFF)
    set(WITH_HEPMC_USER_DISABLED TRUE)
  else()
...
# As for HepMC above, an explicit -DWITH_YODA=OFF is honoured.
if(";${GAMBIT_BITS};" MATCHES ";ColliderBit;")
  if(DEFINED WITH_YODA AND NOT WITH_YODA)
    message("${BoldRed}   WITH_YODA=OFF requested: building ColliderBit without YODA.${ColourReset}")
    set(WITH_YODA OFF)
    set(WITH_YODA_USER_DISABLED TRUE)
  else()
...
if(";${GAMBIT_BITS};" MATCHES ";ColliderBit;")
  ...
  # contrib depends on HepMC, unless the user explicitly disabled it
  if(EXCLUDE_HEPMC)
    if(WITH_HEPMC_USER_DISABLED)
      message("${BoldRed}   Proceeding without HepMC at the user's explicit request; HepMC-dependent ColliderBit functions are disabled.${ColourReset}")
    else()
  ...
  # contrib depends on YODA, unless the user explicitly disabled it
  if(EXCLUDE_YODA)
    if(WITH_YODA_USER_DISABLED)
      message("${BoldRed}   Proceeding without YODA at the user's explicit request; YODA-dependent ColliderBit functions are disabled.${ColourReset}")
    else()
