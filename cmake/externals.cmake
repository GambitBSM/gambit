  ...
  foreach(ditch_command ${itch})
    ...
    if (_loc EQUAL 0)
      if(NOT ditched_${name}_${ver})
        ...
        set(_cbs_announce_ditch TRUE)
        if(CBS_PRESET_BUILD AND GAMBIT_TRIM_USED_BACKENDS)
          set(_cbs_announce_ditch FALSE)
          string(TOLOWER "${name}" _cbs_ditch_name)
          foreach(_cbs_kept ${GAMBIT_TRIM_USED_BACKENDS})
            string(TOLOWER "${_cbs_kept}" _cbs_kept_lower)
            if(_cbs_ditch_name STREQUAL _cbs_kept_lower)
              set(_cbs_announce_ditch TRUE)
              break()
            endif()
          endforeach()
        endif()
        if(_cbs_announce_ditch)
