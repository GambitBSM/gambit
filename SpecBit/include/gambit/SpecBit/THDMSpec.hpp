...
namespace Gambit
{
   namespace SpecBit
   {
      ...
      // wrapper getter method for model type
      template <class Model>
      double get_model_type(const Model&)
      {
        // This doesn't matter cause it's overriden, but this needs to exist for verification purposes
        return 0;
      }
      ...
      template <class Model>
      double get_Yu2(const Model&, int, int)
      {
      ...
      template <class Model>
      double get_Yd2(const Model&, int, int)
      {
        // This is overriden later, so only here for the verification
        return 0;
      }
      ...
      template <class Model>
      double get_Ye2(const Model&, int, int)
      {
        // This is overriden later, so only here for the verification
        return 0;
      }
      ...
      template <class Model>
      double get_ImYu2(const Model&, int, int)
      {
        // This is overriden later, so only here for the verification
        return 0;
      }
      ...
      template <class Model>
      double get_ImYd2(const Model&, int, int)
      {
        // This is overriden later, so only here for the verification
        return 0;
      }
      ...
      template <class Model>
      double get_ImYe2(const Model&, int, int)
      {
        // This is overriden later, so only here for the verification
        return 0;
      }
      ...
      typename THDMSpec<MI>::GetterMaps THDMSpec<MI>::fill_getter_maps()
      {
         ...
         typedef typename MTget::FInfo2M FInfo2M;
         ...
         // (Two-index member functions of model object)
        {
         ...
         // (Zero index, model object as argument)
         {
            typename MTget::fmap0_extraM tmp_map;
            // Model type
            tmp_map["model_type"]= &get_model_type<Model>;
            // Mixing angles
            tmp_map["sinW2"] = &get_sinthW2_DRbar<Model>;
            tmp_map["tanb"]= &get_tanb<Model>;
            tmp_map["alpha"]= &get_alpha<Model>;
            tmp_map["beta"]= &get_beta<Model>;
            // coupling basis dimensionless parameters
            tmp_map["lambda1"]= &get_lambda1<Model>;
            tmp_map["lambda2"]= &get_lambda2<Model>;
            tmp_map["lambda3"]= &get_lambda3<Model>;
            tmp_map["lambda4"]= &get_lambda4<Model>;
            tmp_map["lambda5"]= &get_lambda5<Model>;
            tmp_map["lambda6"]= &get_lambda6<Model>;
            tmp_map["lambda7"]= &get_lambda7<Model>;
            // higgs basis dimensionless parameters
            tmp_map["Lambda1"]= &get_Lambda1<Model>;
            tmp_map["Lambda2"]= &get_Lambda2<Model>;
            tmp_map["Lambda3"]= &get_Lambda3<Model>;
            tmp_map["Lambda4"]= &get_Lambda4<Model>;
         ...
         // Functions utilising the two-index "extraM" function signature
         // (Two-index, model object as argument)
         {
           typename MTget::fmap2_extraM tmp_map;
           tmp_map["Yu1"] = FInfo2M( &get_Yu1<Model>, i012, i012);
           tmp_map["Yd1"] = FInfo2M( &get_Yd1<Model>, i012, i012);
           tmp_map["Ye1"] = FInfo2M( &get_Ye1<Model>, i012, i012);

           tmp_map["ImYu1"] = FInfo2M( &get_ImYu1<Model>, i012, i012);
           tmp_map["ImYd1"] = FInfo2M( &get_ImYd1<Model>, i012, i012);
           tmp_map["ImYe1"] = FInfo2M( &get_ImYe1<Model>, i012, i012);

           tmp_map["Yu2"] = FInfo2M( &get_Yu2<Model>, i012, i012);
           tmp_map["Yd2"] = FInfo2M( &get_Yd2<Model>, i012, i012);
           tmp_map["Ye2"] = FInfo2M( &get_Ye2<Model>, i012, i012);

           tmp_map["ImYu2"] = FInfo2M( &get_ImYu2<Model>, i012, i012);
           tmp_map["ImYd2"] = FInfo2M( &get_ImYd2<Model>, i012, i012);
           tmp_map["ImYe2"] = FInfo2M( &get_ImYe2<Model>, i012, i012);

           map_collection[Par::dimensionless].map2_extraM = tmp_map;
         }
